...
class NonLLMPipeline:
    ...
    def extract_entities(self, text: str) -> dict[str, list[str]]:
        ...
        """
        entities = {"PACKAGE": [], "FILE": [], "ERROR_CODE": [], "SERVICE": []}
    ...
    def analyze_error(self, error_log: str) -> dict[str, any]:
        ...
        if "attribute" in error_log and "missing" in error_log:
            ...
            analysis["suggestion"] = (
                f"Package or attribute not found. Try searching: nix search nixpkgs {entities['PACKAGE'][0] if entities['PACKAGE'] else 'package'}"
            )

        elif "collision" in error_log:
            analysis["type"] = "package_collision"
            analysis["suggestion"] = (
                "Multiple packages provide the same file. Use priority or remove one package."
            )

        elif "read-only file system" in error_log:
            analysis["type"] = "readonly_fs"
            analysis["suggestion"] = (
                "Cannot modify system files directly. Edit configuration.nix instead."
            )

        elif "out of memory" in error_log:
            analysis["type"] = "oom"
            analysis["suggestion"] = (
                "Build ran out of memory. Try: nix-build --cores 1 --max-jobs 1"
            )

        return analysis

    def process_query(
        self, query: str, command_history: list[str] | None = None
    ) -> dict:
        """
        ...
        if command_history:
            ...
            if next_conf > 0.7:
                next_command = {"command": next_cmd, "confidence": next_conf}
...
def demonstrate_pipeline():
    ...
    # Test cases
    test_queries = [
        {"query": "Plz can u get me the fierfox browser", "history": []},
        {
        ...
        {"query": "make my system safe and secure", "history": []},
    ]
    ...
    for test in test_queries:
        ...
        print(
            f"🎯 Intent: {result['intent']['type']} (confidence: {result['intent']['confidence']:.2%})"
        )
        ...
        if result["next_command"]:
            print(
                f"🔮 Predicted next: {result['next_command']['command']} ({result['next_command']['confidence']:.0%} confidence)"
            )
