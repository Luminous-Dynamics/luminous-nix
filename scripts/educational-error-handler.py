...
class CommandError(Exception):
    ...
    def __init__(
        ...
        recovery_commands: list[dict[str, str]] | None = None,
    ):
...
class EducationalErrorHandler:
    ...
    def _handle_package_not_found(
        self, error: str, package: str | None, context: dict
    ) -> CommandError:
    ...
    def _handle_permission_error(
        self, error: str, extracted: str | None, context: dict
    ) -> CommandError:
        """Handle permission errors educationally"""
        user_message = (
            "This action needs special permissions that you don't currently have."
        )
        ...
        recovery_commands.extend(
            [
                {"description": "Check your user groups", "command": "groups"},
                {
                    "description": "Install in user profile instead",
                    "command": (
                        context.get("command", "").replace("sudo ", "")
    ...
    def _handle_network_error(
        ...
        user_message = (
            "I'm having trouble connecting to the internet to complete this task."
        )
        ...
        recovery_commands = [
            {"description": "Test internet connection", "command": "ping -c 3 1.1.1.1"},
            {"description": "Check DNS resolution", "command": "nslookup nixos.org"},
            {
    ...
    def _handle_ssl_error(
        ...
        recovery_commands = [
            {"description": "Check system time", "command": "date"},
            {
    ...
    def _handle_disk_space_error(
        ...
        recovery_commands = [
            {"description": "Check available space", "command": "df -h /"},
            {
    ...
    def _handle_infinite_recursion(
        self, error: str, extracted: str | None, context: dict
    ) -> CommandError:
    ...
    def _handle_dependency_conflict(
        ...
        user_message = (
            "Two packages you have installed need different versions of the same thing."
        )
    ...
    def _handle_channel_error(
        ...
        recovery_commands = [
            {"description": "List current channels", "command": "nix-channel --list"},
            {
                "description": "Try updating again",
                "command": "sudo nix-channel --update",
            },
            {"description": "Check channel URLs", "command": "cat ~/.nix-channels"},
        ]
    ...
    def _handle_unknown_error(self, error: str, context: dict) -> CommandError:
        ...
        user_message = (
            "Something unexpected happened. Let me help you figure out what went wrong."
        )
        ...
        recovery_commands = [
            {"description": "Check system status", "command": "systemctl status"},
            {
    ...
    def format_for_persona(
        self, error: CommandError, persona: str = "default"
    ) -> dict[str, any]:
        ...
        if persona == "grandma_rose":
            ...
            result["suggestions"] = [
                self._simplify_language(s) for s in result["suggestions"][:2]
            ]
            ...
            result["message"] = self._simplify_language(result["message"])
            result["suggestions"] = [
                self._simplify_language(s) for s in result["suggestions"]
            ]
