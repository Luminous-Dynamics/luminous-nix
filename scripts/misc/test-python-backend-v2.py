...
def main():
    ...
    backend_path = (
        Path(__file__).parent / "backend" / "python" / "migrate_to_python_backend.py"
    )
    ...
    tests = [
        ...
        (
            "install --dry-run neofetch",
            "Package installation test (execute mode)",
            False,
        ),
    ]
    ...
    print(
        f"   Dry-run tests: {sum(1 for d, s in results if '(dry-run)' in d and s)}/{dry_run_count}"
    )
    print(
        f"   Execute tests: {sum(1 for d, s in results if '(execute)' in d and s)}/{execute_count}"
    )
