...
def create_backup(files_to_remove, backup_dir: Path):
    ...
    with open(manifest_file, "w") as f:
        f.write("TypeScript/JavaScript Removal Backup\n")
...
def main():
    ...
    backup_dir = (
        root_dir
        / "archive"
        / f'ts-js-removal-{datetime.now().strftime("%Y%m%d_%H%M%S")}'
    )
    ...
    print("\n📊 Summary:")
    print(
        f"  - TypeScript/JavaScript files: {len([f for f in files_to_remove if f.suffix in TS_JS_EXTENSIONS])}"
    )
    print(
        f"  - Config files: {len([f for f in files_to_remove if f.name in CONFIG_FILES])}"
    )
    ...
    if response == "yes":
        ...
        # Offer dry run
        if (
            input("\nWould you like to see what would be removed? (yes/no): ")
            .strip()
            .lower()
            == "yes"
        ):
