...
def fix_test_assertions():
    ...
    for test_file in test_files:
        ...
        content = re.sub(
            r"self\.assertEqual\(intent\.entities\[\'package\'\], ",
            "self.assertEqual(intent.target, ",
            content,
        )
        content = re.sub(
            r"self\.assertIn\(\'package\', intent\.entities\)",
        ...
        content = re.sub(
            r"    async def test_async_recognize.*?(?=    def|\n\nclass|\Z)",
            "",
            content,
            flags=re.DOTALL,
        )
        content = re.sub(
            r"class TestAsyncIntentRecognition.*?(?=\nclass|\Z)",
            "",
            content,
            flags=re.DOTALL,
        )
