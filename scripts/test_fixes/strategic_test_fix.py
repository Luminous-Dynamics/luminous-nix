    ...
    for test_file in learning_tests:
        ...
        if test_path.exists():
            ...
            content = content.replace(
                ...
                "from luminous_nix.learning.learning_system import"
            )
            content = content.replace(
                "from luminous_nix.learning import LearningSystem",
                "from luminous_nix.learning.learning_system import LearningSystem"
            )
            content = content.replace(
                "from scripts.learning",
                "from luminous_nix.learning"
            )
            
            # Add skip if module doesn't exist
            if "luminous_nix.learning.learning_system" in content:
...
def fix_knowledge_base_tests():
    ...
    if test_path.exists():
        ...
        content = content.replace(
            ...
            "from luminous_nix.knowledge.knowledge_base import"
        )
        content = content.replace(
            "from scripts.knowledge",
            "from luminous_nix.knowledge"
        )
        ...
        if "knowledge_base import" in content and "try:" not in content[:200]:
            content = '''import pytest
try:
    from luminous_nix.knowledge.knowledge_base import KnowledgeBase
except ImportError:
    try:
    ...
    for test_file in headless_tests:
        ...
        if test_path.exists():
            ...
            content = content.replace(
                ...
                "from luminous_nix.core.headless_engine"
            )
            content = content.replace(
