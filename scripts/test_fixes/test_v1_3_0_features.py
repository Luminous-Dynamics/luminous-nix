#!/usr/bin/env python3
"""
🧪 Comprehensive Test Suite for v1.3.0 Features
...
def test_fuzzy_search_import() -> Tuple[bool, str]:
    ...
    try:
        from luminous_nix.search import create_searcher, PackageResult
...
def test_fuzzy_search_basic() -> Tuple[bool, str]:
    ...
    try:
        from luminous_nix.search import FuzzySearchAdapter
...
def test_natural_language_expansion() -> Tuple[bool, str]:
    ...
    try:
        from luminous_nix.search import ConsciousFuzzySearch
...
def test_tree_sitter_import() -> Tuple[bool, str]:
    ...
    try:
        from luminous_nix.parsers.multi_language_parser import MultiLanguageAnalyzer
        from luminous_nix.parsers.shell_script_migrator import ShellToNixMigrator
...
def test_project_analysis() -> Tuple[bool, str]:
    ...
    try:
        from luminous_nix.parsers.multi_language_parser import analyze_and_generate
...
def test_cli_search_integration() -> Tuple[bool, str]:
    """Test CLI search command integration"""
    try:
        result = subprocess.run(
            ["poetry", "run", "python", "bin/ask-nix", "search for vim"],
            capture_output=True,
            text=True,
            timeout=10,
        ...
        # Check both stdout and stderr (logs may go to stderr)
        full_output = result.stdout + result.stderr
        
        if "vim" in full_output.lower() and ("found" in full_output.lower() or "packages" in full_output.lower()):
            # Count actual package results
            vim_count = full_output.lower().count("vim")
            return True, f"CLI search found vim ({vim_count} mentions)"
        else:
            # Show a snippet of output for debugging
            snippet = full_output[:500] if full_output else "No output"
            return False, f"No vim in search results: {snippet}"
    except subprocess.TimeoutExpired:
        return False, "CLI search timed out"
    except Exception as e:
        return False, f"CLI error: {e}"

def test_cli_analyze_integration() -> Tuple[bool, str]:
    """Test CLI analyze command integration"""
    try:
        result = subprocess.run(
            ["poetry", "run", "python", "bin/ask-nix", "analyze my project"],
            capture_output=True,
            text=True,
            timeout=10,
            cwd="/srv/luminous-dynamics/11-meta-consciousness/nix-for-humanity"
        ...
        if "python" in full_output.lower() and ("dependencies" in full_output.lower() or "project" in full_output.lower()):
            return True, "CLI analyze command works"
        else:
            snippet = full_output[:500] if full_output else "No output"
            return False, f"Unexpected output: {snippet}"
    except subprocess.TimeoutExpired:
        return False, "CLI analyze timed out"
    except Exception as e:
        return False, f"CLI error: {e}"
...
def test_graceful_degradation() -> Tuple[bool, str]:
    ...
    try:
        from luminous_nix.search.fuzzy_search import FuzzySearchAdapter
...
def main():
    ...
    print("=" * 70)
    print("🧪 v1.3.0 Feature Test Suite")
    print("=" * 70)
    ...
    tests = [
        ...
        ("CLI Search Integration", test_cli_search_integration),
        ("CLI Analyze Integration", test_cli_analyze_integration),
