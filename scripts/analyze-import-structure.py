            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.ImportFrom):
                    ...
                    for alias in node.names:
                        imports.append(
                            {"from": module, "import": alias.name, "as": alias.asname}
                        )
                elif isinstance(node, ast.Import):
                    for alias in node.names:
                        imports.append({"import": alias.name, "as": alias.asname})
    ...
    report = {
        ...
        "statistics": {
            ...
            "relative_imports": sum(
                1
                for imps in imports_by_file.values()
                for imp in imps
                if imp.get("from", "").startswith(".")
            ),
...
def print_report(report):
    ...
    if report["undefined_imports"]:
        ...
        for ui in report["undefined_imports"][:10]:
            print(
                f"  - {ui['file']}: Cannot import '{ui['import']}' from '{ui['from']}'"
            )
