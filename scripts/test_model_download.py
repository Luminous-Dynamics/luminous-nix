...
async def test_model_download():
    ...
    try:
        ...
        if piper_path.exists():
            ...
            print_success("Piper voice model downloaded successfully!")
    ...
    try:
        ...
        success_count = sum(
            1 for r in results if not isinstance(r, Exception) and r.exists()
        )
        print_success(
            f"Downloaded {success_count}/{len(tasks)} models concurrently in {concurrent_time:.2f}s"
        )
...
async def main():
    ...
    if all_passed:
        ...
        print_info(
            "2. Run integration tests: pytest tests/voice/test_voice_integration.py"
        )
