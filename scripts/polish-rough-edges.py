...
class Issue:
    ...
    def __lt__(self, other):
        ...
        return (severity_order.index(self.severity), self.category.value) < (
            severity_order.index(other.severity),
...
class RoughEdgeScanner:
        ...
        for py_file in self._get_python_files():
            try:
                ...
                critical_patterns = [
                    ...
                    (
                        r"subprocess\\.(run|call|check_output)",
                ...
                for line_num, line in enumerate(content.splitlines(), 1):
                    for pattern, desc in critical_patterns:
                        if re.search(pattern, line):
                            ...
                            if not any(
                                word
                                in content.splitlines()[max(0, line_num - 5) : line_num]
    ...
    def _scan_performance_issues(self):
        ...
        performance_patterns = [
            ...
            (
                r"subprocess\\.(run|call).*shell=True",
    ...
    def _generate_report(self):
        ...
        with open(report_path, "w") as f:
            ...
            if high_count > 0:
                f.write(
                    f"**🔧 {high_count} HIGH priority issues should be addressed before release.**\n\n"
                )
            ...
            if priority_issues:
                ...
                for i, issue in enumerate(priority_issues[:10], 1):
                    f.write(
                        f"{i}. **{issue.severity.value}** - {issue.category.value}: "
                        f"{issue.description}\n"
                    )
...
def main():
    ...
    critical_count = sum(
        1 for i in scanner.issues if i.severity == IssueSeverity.CRITICAL
    )
