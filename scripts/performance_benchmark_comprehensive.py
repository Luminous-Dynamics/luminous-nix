...
class PerformanceBenchmark:
    ...
    async def benchmark_command(self, scenario: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            print(
                f"   Duration: {duration_ms:.1f}ms (target: {target_ms:.0f}ms) {status}"
            )
            ...
            target_ms = (
                self.targets.get(scenario.get("target", "simple_command"), 2.0) * 1000
            )

            print(
                f"   ❌ TIMEOUT after {scenario['timeout']}s (target: {target_ms:.0f}ms)"
            )
    ...
    async def run_memory_stress_test(self) -> dict[str, Any]:
        ...
        print(
            f"   Memory Used: {memory_used:.1f}MB (target: {self.targets['memory_active']}MB)"
        )
...
"""
        ...
        maya_result = next(
            (r for r in results if r["scenario"] == "maya_scenario"), None
        )
        if maya_result:
            maya_status = (
                "✅ EXCELLENT" if maya_result["target_met"] else "⚠️ NEEDS ATTENTION"
            )
            report += f"- **Maya (ADHD)**: {maya_result['duration_ms']:.0f}ms response - {maya_status}\n"

        grandma_result = next(
            (r for r in results if r["scenario"] == "grandma_scenario"), None
        )
        if grandma_result:
...
"""
    ...
    async def run_comprehensive_benchmark(self) -> dict[str, Any]:
        ...
        print(
            f"📊 System: {system_info['cpu_count']} cores, {system_info['memory_total_gb']:.1f}GB RAM"
        )
        print(
            f"🐍 Python: {system_info['python_version']} | 💻 NixOS: {system_info['nixos_version']}"
        )
