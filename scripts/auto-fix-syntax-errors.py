...
def fix_file(file_path: Path, error_msg: str):
    ...
    try:
        ...
        if "f-string: expecting" in error_msg:
            ...
            content = re.sub(
                r'f"([^"]*){([^}]*)$', r'f"\1{\2}"', content, flags=re.MULTILINE
            )
            content = re.sub(
                r"f'([^']*){([^}]*)$", r"f'\1{\2}'", content, flags=re.MULTILINE
            )
        ...
        if fixed and content != original_content:
            ...
            print("   ✅ Fixed!")
            return True
        print("   ⚠️  Could not auto-fix this error")
        return False
...
def main():
    ...
    files_to_fix = [
        (
            "scripts/prepare-production-release.py",
            "Line 149: f-string: expecting '!', or ':', or '}'",
        ),
        ...
        (
            ...
            "Line 154: invalid syntax. Perhaps you forgot a comma?",
        ),
        ...
        (
            ...
            "Line 125: unexpected character after line continuation character",
        ),
        (
            "tests/unit/test_backend_comprehensive.py",
            "Line 127: unexpected character after line continuation character",
        ),
        (
            "tests/unit/test_execution_engine_enhanced.py",
            "Line 183: unexpected character after line continuation character",
        ),
        (
            "tests/unit/test_cli_adapter_comprehensive_v2.py",
            "Line 141: cannot assign to function call here. Maybe you meant '==' instead of '='?",
        ),
        (
            "tests/unit/test_nix_integration.py",
            "Line 402: unexpected character after line continuation character",
        ),
    ]
