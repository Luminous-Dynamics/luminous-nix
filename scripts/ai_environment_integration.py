...
class AIEnvironmentIntegration:
    ...
    def handle_environment_request(
        self, query: str, output_dir: str | None = None
    ) -> dict:
    ...
    def _generate_preview(self, requirements: dict, output_path: Path) -> str:
        ...
        if requirements["dev_tools"]:
            preview_lines.append(
                f"  🛠️  Dev tools: {', '.join(requirements['dev_tools'][:3])}..."
            )
    ...
    def format_response(self, response: dict, personality: str = "friendly") -> str:
        ...
        if personality == "encouraging":
            lines.append("")
            lines.append(
                "🌟 You're all set to start your AI/ML journey! The environment will handle all the complex setup for you."
            )
