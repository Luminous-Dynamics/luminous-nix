...
try:
    ...
    class FeedbackCollector:
        ...
        def collect_explicit_feedback(self, **kwargs):
            pass
...
class PersonaFeedbackTester:
    ...
    def _find_ask_nix(self) -> str:
        ...
        try:
            result = subprocess.run(
                ["which", "ask-nix"], capture_output=True, text=True
            )
    ...
    def run_test_scenario(
        self, persona: Persona, scenario: dict[str, Any]
    ) -> TestResult:
        ...
        try:
            ...
            result = subprocess.run(
                ...
                env={
                    **os.environ,
                    "NO_FEEDBACK": "1",
                },  # Disable interactive feedback for testing
            )
    ...
    def _evaluate_response(
        ...
        if "max_response_time" in persona.success_metrics:
            metrics["response_time_ok"] = (
                response_time <= persona.success_metrics["max_response_time"]
            )
        ...
        if "max_word_count" in persona.success_metrics:
            ...
            metrics["word_count_ok"] = (
                word_count <= persona.success_metrics["max_word_count"]
            )
        ...
        if "max_technical_terms" in persona.success_metrics:
            ...
            metrics["technical_terms_ok"] = (
                term_count <= persona.success_metrics["max_technical_terms"]
            )
        ...
        if "screen_reader_compatible" in persona.success_metrics:
            ...
            has_headers = any(
                marker in response for marker in ["#", "**", "Step", "Option"]
            )
            no_ascii_art = not any(
    ...
    def _generate_feedback(
        ...
        if metrics.get("response_time_ok") is False:
            feedback_parts.append(
                f"- Response too slow ({metrics.get('response_time', 0):.1f}s)"
            )

        if metrics.get("word_count_ok") is False:
            feedback_parts.append(
                f"- Too verbose ({metrics.get('word_count', 0)} words)"
            )

        if metrics.get("technical_terms_ok") is False:
            feedback_parts.append(
                f"- Too technical ({metrics.get('technical_terms', 0)} technical terms)"
            )
            ...
            for scenario in persona.test_scenarios:
                ...
                for metric, value in result.metrics.items():
                    if not metric.endswith("_ok"):
                        status = (
                            "✅" if result.metrics.get(f"{metric}_ok", True) else "❌"
                        )
    ...
    def generate_report(self) -> dict[str, Any]:
        ...
        for persona in self.personas:
            ...
            if persona_results:
                success_rate = sum(1 for r in persona_results if r.success) / len(
                    persona_results
                )
                avg_response_time = sum(r.response_time for r in persona_results) / len(
                    persona_results
                )
                ...
                needs_met_rate = (
                    sum(sum(values) / len(values) for values in all_metrics.values())
                    / len(all_metrics)
                    if all_metrics
                    else 0
                )
                ...
                report["persona_results"][persona.name] = {
                    ...
                    "key_issues": [
                        r.feedback for r in persona_results if not r.success
                    ],
                }
    ...
    def _generate_insights(self, report: dict[str, Any]):
        ...
        if report["overall_success_rate"] < 0.8:
            insights.append(
                "⚠️ Overall success rate below 80% - system needs reliability improvements"
            )
        ...
        for persona_name, results in report["persona_results"].items():
            if results["success_rate"] < 0.5:
                insights.append(
                    f"🚨 {persona_name}: Critical issues - only {results['success_rate']:.0%} success rate"
                )
            elif results["needs_met_rate"] < 0.7:
                insights.append(
                    f"⚠️ {persona_name}: Needs not fully met ({results['needs_met_rate']:.0%})"
                )

            if results["avg_response_time"] > 3.0:
                insights.append(
                    f"🐌 {persona_name}: Slow responses ({results['avg_response_time']:.1f}s average)"
                )
        ...
        tech_success = sum(
            report["persona_results"].get(p, {}).get("success_rate", 0)
            for p in technical_personas
        ) / len(technical_personas)
        non_tech_success = sum(
            report["persona_results"].get(p, {}).get("success_rate", 0)
            for p in non_technical_personas
        ) / len(non_technical_personas)

        if tech_success > non_tech_success + 0.2:
            insights.append(
                "📊 System performs better for technical users - needs accessibility improvements"
            )
        elif non_tech_success > tech_success + 0.2:
            insights.append(
                "📊 System performs better for non-technical users - needs power user features"
            )
    ...
    def _generate_improvements(self, report: dict[str, Any]):
        ...
        response_time_issues = sum(
            1 for r in self.results if r.metrics.get("response_time_ok") is False
        )
        ...
        technical_term_issues = sum(
            1 for r in self.results if r.metrics.get("technical_terms_ok") is False
        )
        ...
        screen_reader_issues = sum(
            1 for r in self.results if r.metrics.get("screen_reader_ok") is False
        )
...
def main():
    ...
    if (
        not Path(tester.ask_nix_path).exists()
        and not subprocess.run(
            ["which", tester.ask_nix_path], capture_output=True
        ).returncode
        == 0
    ):
    ...
    try:
        ...
        for improvement in report["actionable_improvements"][:3]:  # Top 3 improvements
            print(
                f"   [{improvement['priority']}] {improvement['area']}: {improvement['action']}"
            )
