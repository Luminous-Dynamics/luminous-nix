...
class CapabilityDetector:
        ...
        if os.path.exists("/etc/nixos/configuration.nix"):
            try:
                result = subprocess.run(
                    ["nixos-version"], capture_output=True, text=True
                )
    ...
    def _detect_hardware(self):
        ...
        self.capabilities["ram_gb"] = round(
    ...
    def _detect_nixos_tools(self):
        ...
        self.capabilities["has_nixos_rebuild_ng"] = self._check_command(
            "nixos-rebuild"
        ) and self._check_python_module("nixos_rebuild")
    ...
    def _detect_voice_engines(self):
        ...
        if self._check_command("piper"):
            ...
            try:
                result = subprocess.run(
                    ["piper", "--list-voices"], capture_output=True, text=True
                )
                if result.returncode == 0:
                    voices = [
    ...
    def _detect_network(self):
        ...
        # Tor service
        self.capabilities["has_tor_service"] = self._check_command(
            "tor"
        ) or os.path.exists("/var/run/tor")
    ...
    def _detect_audio(self):
        ...
        try:
            ...
            self.capabilities["has_audio_input"] = (
                result.returncode == 0 and "card" in result.stdout
            )

            result = subprocess.run(["aplay", "-l"], capture_output=True, text=True)
            self.capabilities["has_audio_output"] = (
                result.returncode == 0 and "card" in result.stdout
            )
...
def save_capabilities(caps: SystemCapabilities, path: str | None = None):
...
def load_capabilities(path: str | None = None) -> SystemCapabilities | None:
...
def main():
    ...
    print("  Ready for Nix for Humanity! 🚀")
