...
class NixOSKnowledgeEngine:
    ...
    def extract_intent(self, query: str) -> dict:
        ...
        if any(
            ...
            return {"action": "install_package", "package": package, "query": query}

        # Search patterns
        if any(
            word in query_lower for word in ["search", "find", "look for", "is there"]
        ):
            return {"action": "search_package", "query": query}

        # Update patterns
        if any(word in query_lower for word in ["update", "upgrade"]):
            return {"action": "update_system", "query": query}
        ...
        if any(
            word in query_lower
            for word in ["generation", "rollback", "previous", "undo"]
        ):
