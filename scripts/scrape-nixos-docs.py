...
class NixOSDocScraper:
    ...
    def parse_manual_page(self, html: str, source_url: str) -> dict | None:
                ...
                code_examples.append(
                    {
                        "code": code,
                        "language": (
                            pre.get("class", [""])[0] if pre.get("class") else "nix"
                        ),
                    }
                )
        ...
        if html:
            ...
            if content:
                ...
                for elem in content.find_all(
                    ["span", "div"], class_=["mw-editsection", "toc"]
                ):
                    elem.decompose()
    ...
    def _extract_wiki_code(self, content):
        ...
        for code_block in content.find_all(["syntaxhighlight", "source", "pre"]):
            ...
            if code:
                code_examples.append({"code": code, "language": lang})
...
def main():
    parser = argparse.ArgumentParser(
        description="Scrape NixOS documentation for model training"
    )
    parser.add_argument(
        "--source",
        choices=["manual", "pills", "wiki", "all"],
        default="all",
        help="Which documentation to scrape",
    )
    parser.add_argument(
        "--output",
        default="training-data/nixos-docs",
