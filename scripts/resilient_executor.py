...
class CommandIntent:
    ...
    target: str | None  # package name, service name, etc.
...
class NixProfileTier(ExecutionTierBase):
    ...
    def execute(self, intent: CommandIntent, dry_run: bool = False) -> ExecutionResult:
        ...
        try:
            ...
            if intent.action == "search" and intent.target:
...
class ResilientExecutor:
    ...
    def __init__(self, capabilities: SystemCapabilities):
        ...
        # User preferences
        self.preferred_tier: ExecutionTier | None = None
    ...
    def execute(
        ...
        dry_run: bool = False,
        tier_override: ExecutionTier | None = None,
    ) -> ExecutionResult:
    ...
    def _select_best_tier(self, intent: CommandIntent) -> ExecutionTierBase | None:
    ...
    def _get_tier_by_type(self, tier_type: ExecutionTier) -> ExecutionTierBase | None:
...
def main():
    ...
    parser.add_argument(
        "--tier",
        choices=["python_api", "nix_profile", "nix_env", "instructions"],
    ...
    parser.add_argument(
        "--json", action="store_true", help="Output JSON for programmatic use"
    )
    ...
    if result.needs_confirmation:
        ...
        if response.lower() == "y":
            ...
            result = executor.execute(
                intent, dry_run=args.dry_run, tier_override=tier_override
            )
        else:
            print("❌ Cancelled")
            return
