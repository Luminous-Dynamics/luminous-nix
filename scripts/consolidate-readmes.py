...
class ReadmeConsolidator:
    ...
    def find_all_readmes(self):
        ...
        for readme in self.project_root.rglob("README.md"):
            ...
            if relative_path == Path("README.md"):  # Root README
                self.readmes_to_keep.append(readme)
            elif relative_path == Path("docs/README.md"):  # Docs root
                self.readmes_to_keep.append(readme)
            elif str(relative_path).startswith("src/"):  # Source code READMEs
                self.readmes_to_keep.append(readme)
            elif "archive" in str(relative_path).lower():  # Archive READMEs
                self.readmes_to_keep.append(readme)
            else:
    ...
    def convert_readme_to_index(self, readme_path):
        ...
        for item in sorted(readme_path.parent.iterdir()):
            if item.is_file() and item.suffix == '.md' and item.name not in ['README.md', 'INDEX.md']:
                index_content += f"- [{item.stem}]({item.name})\n"
                files_added = True
                
        # List subdirectories
        subdirs = [d for d in readme_path.parent.iterdir() if d.is_dir() and not d.name.startswith('.')]
        if subdirs:
            index_content += "\n### 📁 Subdirectories\n\n"
            for subdir in sorted(subdirs):
                # Count .md files in subdir
                md_count = len(list(subdir.rglob("*.md")))
                index_content += f"- [{subdir.name}/]({subdir.name}/) - {md_count} documents\n"
