...
class PackageSearchPlugin(FeaturePlugin):
    ...
    def initialize(self, context: dict[str, Any]) -> bool:
        ...
        try:
            ...
            cache_module_path = os.path.join(
                os.path.dirname(__file__), "..", "..", "package-cache-manager.py"
            )

            if os.path.exists(cache_module_path):
                spec = importlib.util.spec_from_file_location(
                    "package_cache_manager", cache_module_path
                )
    ...
    def handle(self, intent: str, context: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "actions": [
                {"type": "search", "target": search_term, "result_count": len(results)}
            ],
        }
    ...
    def _parse_search_output(self, output: str) -> list[dict[str, Any]]:
        ...
        for line in output.split("\n"):
            if line.startswith("* "):
                ...
                parts = line.split(" ", 2)
                if len(parts) >= 2:
                    current_package = {"name": parts[1].strip(), "description": ""}
