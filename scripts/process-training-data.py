...
class TrainingDataProcessor:
    ...
    def __init__(
        self,
        input_dir: str = "training-data/nixos-docs",
        ...
        self.nixos_patterns = {
            "package_install": re.compile(
                r"(install|add|include)\s+(\w+)\s+(package|program|software)", re.I
            ),
            "configuration": re.compile(
                r"(configuration\.nix|config\.nix|nixos configuration)", re.I
            ),
            ...
            "command": re.compile(
                r"(nixos-rebuild|nix-env|nix-shell|nix develop|nix build)", re.I
            ),
        }
    ...
    def _find_code_context(self, doc: dict, code_snippet: str) -> str:
        ...
        return (
            context
            if context
            else "This code example demonstrates NixOS configuration."
        )
