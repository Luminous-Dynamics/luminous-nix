...
class DocumentationChecker:
    ...
    def check_readme(self):
        ...
        if "7223x faster" in content:
            ...
            if self.check_file_exists("benchmarks/phase2/benchmark_suite.py"):
                ...
                self.results["inaccurate_claims"].append(
                    "Claims 7223x performance but benchmarks not found"
                )
        ...
        features = {
            ...
            "Intelligent Error Handling": [
                ...
                [
                    "src/nix_humanity/core/educational_errors.py",
            ...
            if feature in content:
                ...
                for module, component in locations:
                    if self.check_file_exists(module):
                        if self.check_class_exists(
                            module, component
                        ) or self.check_function_exists(module, component):
                            self.results["accurate_claims"].append(
                                f"{feature} - implementation found in {module}"
                            )
                            found = True
                            break
                        self.results["aspirational_claims"].append(
                            f"{feature} - module {module} exists but implementation incomplete"
                        )
                        found = True
                        break
                if not found:
                    self.results["inaccurate_claims"].append(
                        f"{feature} - claimed but module not found"
                    )
        ...
        for cmd in command_patterns[:5]:  # Test first 5 commands
            ...
            if works:
                self.results["accurate_claims"].append(
                    f"Command works: ask-nix '{cmd}'"
                )
            else:
                self.results["inaccurate_claims"].append(
                    f"Command fails: ask-nix '{cmd}'"
                )
    ...
    def check_user_guide(self):
        ...
        for personality in personalities:
            if f"--{personality}" in content:
                ...
                if self.check_function_exists(
                    "src/nix_humanity/interfaces/cli.py", "enhance_response"
                ):
                    self.results["accurate_claims"].append(
                        f"Personality option --{personality} documented and implemented"
                    )
                else:
                    self.results["aspirational_claims"].append(
                        f"Personality option --{personality} documented but implementation unclear"
                    )
    ...
    def check_quick_start(self):
        ...
        if "./dev.sh" in content:
            if self.check_file_exists("scripts/dev.sh"):
                ...
                self.results["inaccurate_claims"].append(
                    "References ./dev.sh but script not found"
                )
    ...
    def check_working_features(self):
        ...
        if integration_tests.exists():
            for test_file in integration_tests.glob("test_*.py"):
                self.results["accurate_claims"].append(
                    f"Integration test exists: {test_file.name}"
                )
        ...
        if tui_interface.exists():
            with open(tui_interface) as f:
                content = f.read()
                if "def run" in content and "NixHumanityApp" in content:
                    self.results["accurate_claims"].append(
                        "TUI interface is connected to backend"
                    )
    ...
    def generate_recommendations(self):
        ...
        if any(
            "voice" in claim.lower() for claim in self.results["aspirational_claims"]
        ):
            self.results["recommendations"].append(
    ...
    def generate_report(self) -> str:
        ...
        report.append(
            f"- Aspirational claims: {len(self.results['aspirational_claims'])}"
        )
        report.append(
            f"- Missing documentation: {len(self.results['missing_documentation'])}"
        )
        ...
        if self.results["accurate_claims"]:
            ...
            if len(self.results["accurate_claims"]) > 20:
                report.append(
                    f"- ... and {len(self.results['accurate_claims']) - 20} more"
                )
