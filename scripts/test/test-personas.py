...
class PersonaTest:
    ...
    def run_command(
        self, command: str, dry_run: bool = True
    ...
    def evaluate_response(self, output: str, scenario: dict, elapsed: float) -> dict:
        ...
        evaluation = {"passed": True, "issues": [], "strengths": [], "summary": ""}
        ...
        if self.name == "Grandma Rose":
            ...
            if found_jargon:
                evaluation["issues"].append(
                    f"Technical terms: {', '.join(found_jargon)}"
                )
                evaluation["passed"] = False
            ...
            found_complex = [
                word for word in complex_words if word.lower() in output.lower()
            ]
            if found_complex:
                evaluation["issues"].append(
                    f"Complex English: {', '.join(found_complex)}"
                )
...
PERSONAS = [
    ...
    PersonaTest(
        ...
        test_scenarios=[
            ...
            {"description": "Fast search", "command": "find games", "max_time": 2.0},
            {
...
def run_all_tests():
    ...
    for persona in PERSONAS:
        ...
        for result in results:
            ...
            for strength in result["evaluation"]["strengths"]:
                summary["common_strengths"][strength] = (
                    summary["common_strengths"].get(strength, 0) + 1
                )
    ...
    for strength, count in sorted(
        summary["common_strengths"].items(), key=lambda x: x[1], reverse=True
    )[:5]:
