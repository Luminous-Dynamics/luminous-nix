...
class SuccessValidator:
    ...
    def validate_code_quality(self) -> tuple[bool, dict]:
        ...
        if not src_files:
            src_files = (
                list(Path("nix_humanity").rglob("*.py"))
                if Path("nix_humanity").exists()
                else []
            )
    ...
    def validate_documentation(self) -> tuple[bool, dict]:
        ...
        with open("README.md") as f:
            ...
            if not any(
                word in readme
                for word in ["alpha", "beta", "development", "v1.0", "release"]
            ):
        ...
        missing_docs = [
            ...
            if not os.path.exists(doc) and not os.path.exists(f"docs/{doc}")
        ]
    ...
    def validate_testing(self) -> tuple[bool, dict]:
        ...
        integration_tests = [
            f for f in test_files if "integration" in str(f) or "real" in str(f)
        ]
