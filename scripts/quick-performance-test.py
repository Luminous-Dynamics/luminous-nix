...
try:
    ...
    print(
        f"   ✅ Native API: {native_time*1000:.2f}ms ({len(generations)} generations)"
    )
    results["list_generations_native"] = {
        "success": True,
        "time": native_time,
...
try:
    ...
    test_phrases = ["install firefox", "search for text editors", "update my system"]
...
if all_times:
    ...
    print(
        f"All operations < 100ms: {'YES' if all(t < 0.1 for t in all_times) else 'NO'}"
    )
