...
class SacredTrinityRAG:
    ...
    def load_qa_pairs(self) -> list[dict]:
        ...
        if questions_dir.exists() and answers_dir.exists():
            for q_file in questions_dir.glob("q_*.txt"):
                ...
                if a_file.exists():
                    ...
                    qa_pairs.append(
                        {"question": question, "answer": answer, "source": "file"}
                    )
        ...
        for row in c.fetchall():
            qa_pairs.append({"question": row[0], "answer": row[1], "source": "db"})
        conn.close()
    ...
    def retrieve_relevant_qa(self, query: str, count: int = 3) -> list[dict]:
        ...
        for keyword in keyword_list:
            ...
            for row in c.fetchall():
                relevant_qa.append(
                    {"question": row[0], "answer": row[1], "score": row[2]}
                )
    ...
    def create_rag_prompt(
        self, query: str, model_type: str, examples: list[dict]
    ) -> str:
        ...
        for model_type in self.model_configs:
            ...
            if current_file.exists():
                ...
                for query in test_queries[:2]:  # Test first 2 queries
                    ...
                    print(
                        f"💬 {answer[:200]}..." if len(answer) > 200 else f"💬 {answer}"
                    )
...
if __name__ == "__main__":
    ...
    nixos_qa = Path(
        "/srv/luminous-dynamics/11-meta-consciousness/nix-for-humanity/scripts/nixos-core-qa.txt"
    )
    if nixos_qa.exists():
