...
class AIEnvironmentGenerator:
    ...
    def create_environment(
        self, response: dict, target_dir: str | None = None, force: bool = False
    ) -> tuple[bool, str]:
        """
        ...
        if output_dir.exists() and not force:
            if any(output_dir.iterdir()):
                return (
                    False,
                    f"Directory {output_dir} already exists and is not empty. Use --force to overwrite.",
                )
    ...
    def validate_environment(self, env_dir: str) -> tuple[bool, str]:
        ...
        try:
            ...
            if missing:
                return (
                    False,
                    f"Flake.nix missing required elements: {', '.join(missing)}",
                )
...
"""
    ...
    def create_example_notebook(
        self, env_dir: str, model_type: str
    ) -> tuple[bool, str]:
