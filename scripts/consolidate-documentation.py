...
class DocumentationConsolidator:
    def __init__(self, docs_root):
        ...
        self.backup_dir = (
            self.docs_root / f"backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
        self.duplicates = []
        self.moved_files = []
    ...
    def generate_report(self):
        ...
        with open(summary_path, "w") as f:
            f.write("# Documentation Cleanup Summary\n\n")
...
def main():
    ...
    print("Review the changes and run 'git add -A && git commit' when satisfied.")
