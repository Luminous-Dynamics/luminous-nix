...
class CoverageMonitor:
    ...
    def _categorize_file(self, file_path: str) -> tuple[str, float]:
        ...
        if any(path in file_path for path in CORE_FEATURES):
            return ("core", CORE_FEATURE_THRESHOLD)
        if any(path in file_path for path in UI_COMPONENTS):
    ...
    def show_gaps(self, results: dict):
        ...
        for category in ["critical", "core", "ui", "other"]:
            if category in gaps_by_category:
                ...
                for gap in sorted(
                    gaps_by_category[category], key=lambda x: x["coverage"]
                ):
                    print(
    ...
    def generate_report(self, results: dict, output_file: Path | None = None):
        ...
        if all_passed:
            report.append(
                "✅ All coverage thresholds met! Continue maintaining high standards."
            )
        else:
            report.append("### Immediate Actions:")
            if results["critical"] < CRITICAL_PATH_THRESHOLD:
                report.append(
                    "1. **Focus on critical paths** - These are safety-critical components"
                )
            if results["core"] < CORE_FEATURE_THRESHOLD:
                report.append(
                    "2. **Improve core feature coverage** - Essential for reliability"
                )
            if results["overall"] < OVERALL_THRESHOLD:
                report.append(
                    "3. **Increase overall coverage** - Aim for comprehensive testing"
                )
...
def main():
    ...
    parser.add_argument(
        "--trends", action="store_true", help="Show coverage trends over time"
    )
    parser.add_argument("--report", type=Path, help="Generate detailed report to file")
    parser.add_argument(
        "--ci", action="store_true", help="CI mode - fail if thresholds not met"
    )
