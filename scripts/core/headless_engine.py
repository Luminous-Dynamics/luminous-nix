...
try:
    ...
    spec = importlib.util.spec_from_file_location(
        "package_cache_manager", os.path.join(parent_dir, "package-cache-manager.py")
    )
...
class HeadlessEngine:
    ...
    def process(self, input_text: str, context: Context = None) -> Response:
        ...
        # Format response
        response_text = self.knowledge.format_response(intent.__dict__, solution)
    ...
    def _calculate_confidence(self, raw_intent: dict[str, Any]) -> float:
        ...
        if raw_intent.get("action") == "unknown":
            return 0.3
        if raw_intent.get("package"):
            return 0.9
        return 0.7

    def _try_plugin_handling(self, intent: Intent, context: Context) -> Response | None:
    ...
    def _generate_visual(
        self, solution: dict[str, Any], context: Context
    ) -> dict[str, Any] | None:
    ...
    def _generate_feedback_request(self, context: Context) -> dict[str, Any] | None:
    ...
    def _track_interaction(self, intent: Intent, response: Response, context: Context):
        ...
        if self.learning and intent.action == "install_package":
            ...
            self.learning.record_command(
                intent.action, intent.query, f"install {intent.package}"
            )
...
if __name__ == "__main__":
    ...
    for query in test_queries:
        ...
        # Create context
        context = Context(personality="friendly", capabilities=["text", "visual"])
