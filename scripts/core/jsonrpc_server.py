...
class JSONRPCServer:
    ...
    def __init__(
        self, engine: HeadlessEngine, socket_path: str = None, tcp_port: int = None
    ):
        """
    ...
    def _handle_client(self, client_socket: socket.socket):
        ...
        try:
            ...
            error_response = self._create_error_response(
                None, self.INTERNAL_ERROR, str(e)
            )
    ...
    def _process_request(self, data: str) -> str:
        ...
        try:
            ...
            if not isinstance(request, dict):
                response = self._create_error_response(
                    None, self.INVALID_REQUEST, "Request must be an object"
                )
                return json.dumps(response)
            ...
            if "method" not in request:
                response = self._create_error_response(
                    request.get("id"), self.INVALID_REQUEST, "Missing method"
                )
                return json.dumps(response)
            ...
            try:
                ...
                response = self._create_success_response(request.get("id"), result)
                return json.dumps(response)
            except JSONRPCError as e:
                response = self._create_error_response(
                    request.get("id"), e.code, e.message, e.data
                )
                return json.dumps(response)
            except Exception as e:
                logger.error(f"Method execution error: {e}")
                response = self._create_error_response(
                    request.get("id"), self.INTERNAL_ERROR, str(e)
                )
                return json.dumps(response)
            ...
            response = self._create_error_response(
                None, self.INTERNAL_ERROR, "Internal error"
            )
            return json.dumps(response)
    ...
    def _handle_process(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        if not input_text:
            raise JSONRPCError(self.INVALID_PARAMS, "Missing 'input' parameter")
    ...
    def _handle_feedback(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        if not session_id:
            raise JSONRPCError(self.INVALID_PARAMS, "Missing 'session_id' parameter")

        feedback = params.get("feedback")
        if not feedback:
            raise JSONRPCError(self.INVALID_PARAMS, "Missing 'feedback' parameter")
        ...
        return {
            "success": success,
            "message": (
                "Feedback collected" if success else "Failed to collect feedback"
            ),
        }
    ...
    def _handle_echo(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        return {"echo": params, "server": "nix-for-humanity-headless"}

    def _handle_list_methods(self, params: dict[str, Any]) -> dict[str, Any]:
        """Handle 'list_methods' method"""
        return {"methods": list(self.methods.keys()), "version": "1.0.0"}
...
if __name__ == "__main__":
    ...
    result = client.call(
        "process",
        {
            ...
            "context": {"personality": "minimal", "capabilities": ["text", "visual"]},
        },
    )
    print("\nProcess test:")
