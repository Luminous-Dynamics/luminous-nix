...
class UnifiedNixBackend:
    ...
    def _handle_create_flake(
        ...
        try:
            ...
            return BackendResponse(
                success=False, text=f"❌ Error creating flake: {str(e)}", intent=intent
            )
    ...
    def _handle_check_health(
        ...
        try:
            ...
            disk_emoji = (
                ...
                else "⚠️" if health.disk_usage_percent < 90 else "❌"
            )
            ...
            mem_emoji = (
                ...
                else "⚠️" if health.memory_usage_percent < 90 else "❌"
            )
    ...
    def _handle_create_snapshot(
        ...
        try:
            ...
            if success:
                text = f"✅ {message}\n\n"
                text += "📸 Snapshot created with description:\n"
                text += f'   "{description}"\n\n'
    ...
    def _handle_analyze_log(
        ...
        try:
            ...
            text += (
                "💡 Use 'ask-nix error explain' on specific errors for detailed help."
            )
    ...
    def _handle_apply_theme(
        ...
        try:
            ...
            return BackendResponse(
                success=False, text=f"❌ Error applying theme: {str(e)}", intent=intent
            )
    ...
    def _handle_sync_configs(
        ...
        try:
            ...
            return BackendResponse(
                success=False, text=f"❌ Error with sync: {str(e)}", intent=intent
            )
    ...
    def _handle_discover_package(
        ...
        try:
            ...
            if not matches:
                ...
                text += (
                    "  • Searching by command with 'which package provides <command>'"
                )
            ...
            if alternatives:
                text += (
                    f"🔄 Similar to {matches[0].name}: {', '.join(alternatives[:3])}"
                )
    ...
    def _handle_find_by_command(
        ...
        try:
            ...
            text += "💡 Quick install the most likely match:\n"
            ...
            return BackendResponse(
                success=False, text=f"❌ Error finding command: {str(e)}", intent=intent
            )
