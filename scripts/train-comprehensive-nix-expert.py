...
Always prefer declarative approaches and explain the 'why' behind recommendations.""",
    ...
    def download_comprehensive_docs(self) -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                [sys.executable, "download-nix-documentation.py"],
                capture_output=True,
                text=True,
            )
    ...
    def process_theory_documents(self) -> bool:
        ...
        try:
            result = subprocess.run(
                [sys.executable, "process-nix-theory.py"],
                capture_output=True,
                text=True,
            )
    ...
    def create_unified_training_set(self):
        ...
        theory_files = ["nix_master_qa.json", "practical_qa.json"]
    ...
    def create_training_formats(self, qa_data: list[dict]):
        ...
        alpaca_data = []
        for qa in qa_data:
            alpaca_data.append({"instruction": qa["q"], "input": "", "output": qa["a"]})
        ...
        print("✓ Created training formats: Alpaca, ShareGPT, Ollama")
...
"""
    ...
    def create_model_with_ollama(self) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                print(
                    f"✓ Model created successfully: {self.model_config['model_name']}"
                )
                return True
    ...
    def test_model(self):
        ...
        for question in test_questions[:3]:  # Test first 3 questions
            ...
            try:
                result = subprocess.run(
                    ["ollama", "run", self.model_config["model_name"], question],
                    capture_output=True,
                    text=True,
                    timeout=30,
                )
...
def main():
    ...
    parser.add_argument(
        "--skip-download", action="store_true", help="Skip downloading documentation"
    )
