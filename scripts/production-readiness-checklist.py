...
class ProductionReadinessChecker:
    ...
    def _check_core_functionality(self):
        ...
        for cmd, desc in core_commands:
            try:
                result = subprocess.run(
                    cmd.split(), capture_output=True, timeout=5, cwd=self.project_root
                )
        ...
        if broken:
            self.checklist.append(
                ChecklistItem(
                    ...
                    action="Fix broken commands: "
                    + ", ".join([desc for _, desc in broken]),
        ...
        native_backend_file = (
            self.project_root / "src/nix_humanity/nix/native_backend.py"
        )
        ...
        if security_issues:
            self.checklist.append(
                ChecklistItem(
                    ...
                    + ", ".join(
                        [
                            f"{risk} ({count} instances)"
                            for risk, count in security_issues
                        ]
                    ),
                    estimated_time="2-4 hours",
                )
    ...
    def _generate_checklist(self):
        ...
        # Calculate totals
        total_time = 0
        time_by_priority = dict.fromkeys(Priority, 0)
        ...
        for priority in priority_order:
            ...
            if count > 0:
                print(
                    f"{priority.value}: {count} items (~{time_by_priority[priority]} hours)"
                )
        ...
        with open(report_path, "w") as f:
            ...
            f.write(
                f"- Blocker issues: {sum(1 for i in self.checklist if i.priority == Priority.BLOCKER)}\n\n"
            )
