...
class ResilientExecutor:
    ...
    def initialize(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["nixos-rebuild", "--version"], capture_output=True, text=True
            )
        ...
        try:
            result = subprocess.run(
                ["nix", "profile", "--version"], capture_output=True, text=True
            )
    ...
    def execute_with_fallback(self, command: dict) -> dict:
        ...
        return {"success": False, "message": "All execution methods failed"}
...
class ResilientCore:
    ...
    def _assess_capabilities(self) -> SystemCapabilities:
        ...
        if (
            self.voice.stt_engine
            and self.voice.tts_engine
            and self.nlp.active_tier.__class__.__name__ == "MistralLLMTier"
        ):
            rating = "Premium"
        ...
        return SystemCapabilities(
            ...
            nlp_tier=(
                self.nlp.active_tier.__class__.__name__
                if self.nlp.active_tier
                else "None"
            ),
            executor_tier=(
                self.executor.active_tier["name"]
                if self.executor.active_tier
                else "None"
            ),
    ...
    async def process_input(self, input_text: str | None = None) -> dict:
