...
class ContextAwareness:
    ...
    def assess_current_context(self) -> UserContext:
        ...
        work_context = self._determine_work_context(
            activity_context,
            cognitive_state
        )
    ...
    def _determine_work_context(self, activity_context: Dict,
                              cognitive_state: Dict) -> WorkContext:
        ...
        elif pattern_type == 'focused_work':
            return WorkContext.SHALLOW_WORK
            
        elif pattern_type == 'exploring' and cognitive_state.get('learning_momentum', 0) > 0.5:
            return WorkContext.LEARNING
            
        elif pattern_type == 'context_switching' and cognitive_state.get('frustration_level', 0) > 0.5:
            return WorkContext.TROUBLESHOOTING
            
        elif pattern_type == 'break_time':
            return WorkContext.BREAK
            
        elif 'browser' in str(activity_context.get('recent_patterns', [])):
    ...
    def _calculate_energy_level(self, time_context: TimeContext,
                              activity_context: Dict) -> float:
    ...
    def _calculate_stress_level(self, cognitive_state: Dict,
                              activity_context: Dict) -> float:
    ...
    def _calculate_focus_potential(self, energy: float, stress: float,
                                 time_context: TimeContext) -> float:
    ...
    def _calculate_interruption_cost(self, work_context: WorkContext,
                                   focus_potential: float,
                                   activity_context: Dict) -> float:
    ...
    def _recommend_task_type(self, work_context: WorkContext,
                           energy: float, focus_potential: float) -> str:
    ...
    def _calculate_prediction_confidence(self) -> float:
        ...
        stability = self.current_context.context_stability if self.current_context else 0.5
    ...
    def get_context_recommendations(self) -> List[str]:
        ...
        if self.current_context.energy_level < 0.3:
            recommendations.append("Your energy is low. Consider taking a break or switching to routine tasks.")
            
        # Stress-based recommendations
        if self.current_context.stress_level > 0.7:
            recommendations.append("High stress detected. Try a breathing exercise or brief walk.")
            
        # Focus-based recommendations
        if self.current_context.focus_potential > 0.7:
            if self.current_context.work_context != WorkContext.DEEP_WORK:
                recommendations.append("Excellent focus potential! This is a great time for complex tasks.")
                
        # Context-specific recommendations
        if self.current_context.work_context == WorkContext.DEEP_WORK:
            recommendations.append("You're in deep work mode. Protect this time from interruptions.")
        elif self.current_context.work_context == WorkContext.TROUBLESHOOTING:
            recommendations.append("Troubleshooting can be frustrating. Remember to take breaks.")
