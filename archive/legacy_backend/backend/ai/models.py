...
class ModelManager:
    ...
    def whisper(self):
        ...
        if not self._available_models.get('whisper'):
            raise ImportError("Whisper is not available. Install with: nix-env -iA nixpkgs.whisper-cpp")
    ...
    def embeddings(self):
        ...
        if not self._available_models.get('embeddings'):
            raise ImportError("Sentence transformers not available. Install with: nix-env -iA nixpkgs.python311Packages.sentence-transformers")
    ...
    def spacy_nlp(self):
        ...
        if not self._available_models.get('spacy'):
            raise ImportError("spaCy not available. Install with: nix-env -iA nixpkgs.python311Packages.spacy")
