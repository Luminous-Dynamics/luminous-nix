...
from nix_humanity.xai.models import CausalFactor, Explanation, AlternativeExplanation
...
def demo_install_decision():
    ...
    print("• User need match: You asked for a web browser and Firefox perfectly matches this need")
    ...
    print("• User preference: You prefer open-source software, and Firefox is fully open-source")
    print("")
    ...
    factors = [
        CausalFactor("user_need_match", "web_browser", 0.9, 0.95, 
                    "Direct match for browser requirement"),
        CausalFactor("open_source_preference", True, 0.8, 0.90,
                    "Firefox aligns with open-source preference"),
        CausalFactor("package_availability", True, 0.7, 1.0,
                    "Package exists in nixpkgs"),
        CausalFactor("system_compatibility", True, 0.6, 0.95,
                    "System meets all requirements"),
        CausalFactor("community_support", "strong", 0.5, 0.85,
                    "Large user community")
    ]
    
    for factor in factors:
        print(f"  • {factor.name}: {factor.value}")
        print(f"    Influence: {factor.influence:+.1f}, Confidence: {factor.confidence:.0%}")
