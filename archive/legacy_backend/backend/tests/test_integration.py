...
class TestFullStackIntegration:
    ...
    async def test_error_recovery_integration(self, backend):
        ...
        with patch('core.nix_integration.NixOSIntegration.execute_intent') as mock_execute:
...
class TestEndToEndScenarios:
    ...
    async def test_new_user_onboarding(self):
        ...
        first_command = Request(
            text="I want to install a text editor",
            context={'first_time_user': True}
        )
        ...
        assert any(editor in first_response.explanation.lower() 
    ...
    async def test_troubleshooting_flow(self):
        ...
        assert response.success
        assert len(response.suggestions) > 0
        assert 'network' in response.explanation.lower() or 'wifi' in response.explanation.lower()
