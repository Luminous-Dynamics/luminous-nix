...
from .models import Decision, CausalNode, CausalEdge
...
class CausalModelBuilder:
    ...
    def build_model(self, decision: Decision) -> CausalModel:
        ...
        if not model_template:
            raise ValueError(f"No causal model template for decision type: {decision_type}")
    ...
    def _classify_decision(self, decision: Decision) -> str:
        ...
        if 'package' in decision.context:
            ...
            elif decision.action in ['remove', 'delete']:
            ...
            if node_type == 'boolean':
                ...
                if node_id in decision.context:
                    ...
                    if actual in possible_values:
                        probs = [0.7 if v == actual else 0.3/(len(possible_values)-1) 
                ...
                data[node_id] = np.random.choice(possible_values, size=n_samples, p=probs)
    ...
    def _identify_common_causes(
        ...
        for node in graph.nodes():
            ...
            affects_treatment = (
                graph.has_edge(node, treatment) or
                nx.has_path(graph, node, treatment)
            )
            
            # Check if node affects outcome
            affects_outcome = (
                graph.has_edge(node, outcome) or
                nx.has_path(graph, node, outcome)
            )
