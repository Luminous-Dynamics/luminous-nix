...
from .models import (
    Decision, Explanation, CausalFactor, AlternativeExplanation, 
    ExplanationLevel, CausalEffects, CausalRelationship
)
...
class ExplanationGenerator:
    ...
    def generate_explanation(
        ...
        expert = self._generate_expert_explanation(decision, causal_effects, causal_path)
    ...
    def _generate_simple_explanation(
        ...
        primary_factor = max(factors, key=lambda f: abs(f.influence) * f.confidence) if factors else None
    ...
    def _generate_detailed_explanation(
        ...
        if causal_path:
            ...
            explanation_parts.append(f"The decision follows this reasoning: {path_description}")
        ...
        if confidence > 0.8:
            ...
            explanation_parts.append("There's some uncertainty, but this seems like the best option.")
    ...
    def _analyze_alternatives(
        ...
        for alt in alternatives:
            if alt.get("action") == decision.action and alt.get("target") == decision.target:
    ...
    def _get_rejection_reason(self, chosen: Decision, alternative: Dict[str, Any]) -> str:
        ...
        elif alternative.get('risk', 0) > 0.7:
            return "Too risky compared to chosen option"
        elif alternative.get('complexity', 0) > 0.8:
