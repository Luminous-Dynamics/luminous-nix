...
try:
    ...
    print("Warning: tree-sitter not available. Install with: pip install tree-sitter tree-sitter-nix")
...
class NixParser:
    ...
    def parse_content(self, content: bytes, source_name: str = "<input>") -> NixConfigAnalysis:
    ...
    def _walk_tree(self, node, content: bytes, analysis: NixConfigAnalysis):
        ...
        if node.type == "apply" and node.child_count >= 2:
            if node.children[0].type == "identifier" and self._get_text(node.children[0], content) == "import":
        ...
        if node.type == "list":
            for child in node.children:
                if child.type == "identifier" or child.type == "attrpath":
                    ...
                    if name and not name in ["[", "]", "with", "pkgs"]:
                        package = NixPackage(
                            name=name.split(".")[-1],
                            attribute_path=name,
                            source_location=(child.start_point[0], child.start_point[1])
                        )
                        packages.append(package)
                        
        return packages
        
    def _extract_function(self, node, content: bytes) -> Optional[NixFunction]:
    ...
    def _extract_import(self, node, content: bytes) -> Optional[NixImport]:
    ...
    def _is_service_config(self, node) -> bool:
        ...
        if parent and parent.type == "binding":
            attr_path = self._get_attribute_path(parent)
            return any(pattern.replace("*", "") in attr_path for pattern in self.service_patterns)
            
        return False
        
    def _extract_service(self, node, content: bytes) -> Tuple[Optional[str], Dict]:
    ...
    def _get_binding_key(self, binding_node, content: bytes) -> Optional[str]:
...
class NixIntelligence:
    ...
    def suggest_package_installation(self, package_name: str, current_config: str) -> str:
        ...
        has_system_packages = any("systemPackages" in p.attribute_path 
...
"""
    ...
    def _generate_suggestions(self, analysis: NixConfigAnalysis) -> List[str]:
        ...
        if not analysis.packages and not analysis.services:
            suggestions.append("Configuration appears empty. Consider adding packages or services.")
        ...
        for service_name, config in analysis.services.items():
            if isinstance(config, dict) and config.get('enable') == 'true':
                if not config.get('DynamicUser'):
                    suggestions.append(f"Consider using DynamicUser for service '{service_name}'")
