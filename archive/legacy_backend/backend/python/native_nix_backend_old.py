...
class NativeNixBackend:
    ...
    async def execute(self, operation: NixOperation) -> NixResult:
        ...
        try:
            ...
            elif operation.type == OperationType.ROLLBACK:
                return await self._rollback_system(operation)
            elif operation.type == OperationType.INSTALL:
            ...
            elif operation.type == OperationType.SEARCH:
                return await self._search_packages(operation)
            elif operation.type == OperationType.BUILD:
                return await self._build_system(operation)
            elif operation.type == OperationType.TEST:
                return await self._test_configuration(operation)
            elif operation.type == OperationType.LIST_GENERATIONS:
    ...
    async def _rollback_system(self, operation: NixOperation) -> NixResult:
        ...
        try:
            ...
            if target_generation:
                ...
                if not self._validate_generation_exists(target_generation, generations):
                    return NixResult(
                        ...
                        data={"available_generations": [g["number"] for g in generations]}
                    )
            ...
            comparison = await self._compare_generations(current_gen, selected_generation)
            ...
            if NATIVE_API_AVAILABLE:
                ...
                await self._run_subprocess([
                    ...
                    "--switch-generation", selected_generation
                ], timeout=300)
            ...
            if "permission denied" in error_message.lower():
                ...
                user_message = "Target generation not found - it may have been garbage collected"
    ...
    def _validate_generation_exists(self, target_gen: str, generations: List[Dict[str, Any]]) -> bool:
    ...
    async def _find_generation_by_description(self, description: str, generations: List[Dict[str, Any]]) -> Optional[str]:
        ...
        try:
            ...
            try:
                ...
                gen1_result = await self._run_subprocess([
                    "nix-store", "--query", "--tree", f"/nix/var/nix/profiles/system-{gen1}-link"
                ], timeout=10)
                
                gen2_result = await self._run_subprocess([
                    "nix-store", "--query", "--tree", f"/nix/var/nix/profiles/system-{gen2}-link"
                ], timeout=10)
                
                # Basic package difference detection
                gen1_packages = set(line.strip() for line in gen1_result.stdout.split('\n') if 'nixpkgs' in line)
                gen2_packages = set(line.strip() for line in gen2_result.stdout.split('\n') if 'nixpkgs' in line)
                
                added = gen2_packages - gen1_packages
                removed = gen1_packages - gen2_packages
                
                comparison["package_changes"]["added"] = list(added)[:10]  # Limit to first 10
                comparison["package_changes"]["removed"] = list(removed)[:10]
                
                if added or removed:
                    comparison["differences"].append(f"{len(added)} packages added, {len(removed)} removed")
                else:
                    comparison["differences"].append("No significant package changes detected")
                    
            except Exception as query_error:
                logger.warning(f"Could not analyze generation differences: {query_error}")
...
Then run: nixos-rebuild switch"""
    ...
    async def _search_packages(self, operation: NixOperation) -> NixResult:
        ...
        try:
            ...
            result = await self._run_subprocess(["nix", "search", "nixpkgs", query], timeout=60)
    ...
    async def _run_subprocess(self, cmd: List[str], timeout: int = 300) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(f"Running subprocess: {' '.join(shlex.quote(arg) for arg in cmd)}")
    ...
    async def _fallback_rollback_system(self, operation: NixOperation) -> NixResult:
        ...
        try:
            result = await self._run_subprocess(["sudo", "nixos-rebuild", "switch", "--rollback"])
    ...
    async def _fallback_build_system(self, operation: NixOperation) -> NixResult:
        ...
        try:
            result = await self._run_subprocess(["sudo", "nixos-rebuild", "build"], timeout=600)
    ...
    async def _fallback_test_configuration(self, operation: NixOperation) -> NixResult:
        ...
        try:
            result = await self._run_subprocess(["sudo", "nixos-rebuild", "test"], timeout=600)
    ...
    async def _fallback_list_generations(self, operation: NixOperation) -> NixResult:
        ...
        try:
            result = await self._run_subprocess(["sudo", "nixos-rebuild", "list-generations"])
