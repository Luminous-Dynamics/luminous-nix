...
try:
    from src.nix_for_humanity.security.enhanced_validator import EnhancedInputValidator, ValidationContext
except ImportError:
...
class CommandExecutor:
    ...
    def __init__(self, mode: ExecutionMode = ExecutionMode.INTERACTIVE, security_level: str = "balanced"):
        self.mode = mode
    ...
    def execute_command(self, action: str, package: str = None, user_context: Dict = None) -> Dict:
        ...
        if self.validator and ValidationContext:
            ...
            context = ValidationContext(
                ...
                trust_level=user_context.get('trust_level', 0.5) if user_context else 0.5,
                command_history=user_context.get('command_history', []) if user_context else []
            )
    ...
    def _install_package(self, package: str) -> Dict:
        ...
        if self.validator:
            try:
                ...
                package = sanitized.value if isinstance(sanitized.value, str) else str(sanitized.value)
        ...
        if result['success']:
            ...
            if package in ['nodejs', 'python3', 'rust']:
                result['note'] = "You may need to restart your shell or run 'source ~/.bashrc'"
    ...
    def _update_system(self) -> Dict:
        ...
        # Update channels
        channel_result = self._run_command(['sudo', 'nix-channel', '--update'], needs_sudo=True)
    ...
    def _rollback_system(self) -> Dict:
        ...
        if self.mode == ExecutionMode.INTERACTIVE:
            ...
            gen_result = self._run_command(['sudo', 'nix-env', '--list-generations', 
