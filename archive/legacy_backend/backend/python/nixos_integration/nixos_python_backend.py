...
class NixOSPythonBackend:
                ...
                for py_path in possible_paths:
                    if Path(py_path).exists():
                        ...
                        try:
                            ...
                            logger.info(f"✅ Found nixos-rebuild Python API at {py_path}")
                            return True
                        except ImportError:
            ...
            for line in result.stdout.splitlines():
                if 'site-packages' in line:
                    ...
                    try:
                        ...
                        self.nixos_rebuild_module = nixos_rebuild
                        self.has_python_api = True
                        logger.info(f"✅ Found nixos-rebuild Python API at {parent_path}")
                        return True
                    except ImportError:
    ...
    async def _execute_via_api(self, action: Action, **kwargs) -> BuildResult:
        ...
        try:
            ...
            # Get profile
            profile = models.Profile.from_arg(
                kwargs.get('profile_name', 'system')
            )
    ...
    async def install_package(self, package: str, method: str = "profile") -> BuildResult:
        """
