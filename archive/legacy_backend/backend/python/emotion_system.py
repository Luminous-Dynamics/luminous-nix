...
class EmotionSystem:
    ...
    def update_from_nlp_confidence(self, confidence: float, intent: str = None) -> EmotionState:
        """Update emotion based on NLP confidence score"""
        # Find appropriate emotion for confidence level
        emotion = EmotionType.CALM
        for threshold, emotion_type in sorted(self.confidence_thresholds.items(), reverse=True):
        ...
        if confidence >= 0.8:
            ...
            intensity = 0.7 + (0.4 - confidence) * 0.75  # 0.7-1.0 for low (more intense confusion)
    ...
    def blend_emotions(self, primary: EmotionType, secondary: EmotionType, 
                      blend_factor: float = 0.3) -> EmotionState:
    ...
    def get_animation_params(self) -> Dict:
        ...
        params['pulse_rate'] = 1.0 + (params['pulse_rate'] - 1.0) * self.current_emotion.intensity
    ...
    def return_to_calm(self):
        ...
        if self.current_emotion.primary != EmotionType.CALM:
            ...
            if self.current_emotion.intensity > 0.5:
                ...
                # Then switch to calm
                self._update_emotion(EmotionState(
...
class EmotionNLPConnector:
    ...
    def process_nlp_result(self, intent: str, confidence: float, 
                          success: bool = None, error: str = None) -> Dict[str, any]:
        """Process NLP result and update emotions accordingly"""
        
        # Update based on confidence
        emotion_state = self.emotion_system.update_from_nlp_confidence(confidence, intent)
        
        # Additional event-based updates
        if error:
            emotion_state = self.emotion_system.update_from_event('error', {'error': error})
        elif success is True:
            emotion_state = self.emotion_system.update_from_event('success', {'operation': intent})
        elif confidence > self.last_confidence + 0.2:
            # Significant confidence increase - learning!
            emotion_state = self.emotion_system.update_from_event('learning', {'pattern': intent})
...
if __name__ == "__main__":
    ...
    test_cases = [
        ...
        {"intent": "unknown command", "confidence": 0.15, "error": "Intent not recognized"},
    ...
    for test in test_cases:
        ...
        print(f"Animation: pulse={result['animation']['pulse_rate']:.1f}, "
    ...
    interaction = [
        ...
        {"stage": "complete", "intent": "install vscode", "confidence": 0.98, "success": True}
    ]
    ...
    print("\nEmotional Journey:")
    for step in journey:
        print(f"  {step['stage']}: {step['emotion']['emotion']} "
