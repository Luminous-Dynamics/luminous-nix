...
class TrustEngine:
    ...
    def process_interaction(self, interaction_id: str, 
    ...
    def _determine_trust_state(self, trust_level: float, 
                              repair_needed: bool) -> TrustState:
    ...
    def _transition_trust_state(self, new_state: TrustState, 
                               interaction_id: str):
        ...
        self.logger.info(f"Trust state transition: {old_state.value} → {new_state.value}")
    ...
    def _record_trust_update(self, interaction_id: str, 
                           trust_context: Dict):
    ...
    def express_vulnerability(self, vulnerability_type: str,
                            context: Dict) -> Dict:
        """
    ...
    def initiate_repair(self, error_context: Dict) -> Dict:
        ...
        repair_strategy = self.repair.generate_repair_strategy(
            error_context,
            self.current_state,
            self.mental_model.get_user_state()
        )
    ...
    def _analyze_trust_trajectory(self) -> str:
        ...
        if all(s in ['established', 'deep'] for s in states):
            return "stable_high"
        elif 'damaged' in states or 'repairing' in states:
            return "recovering"
        elif states[-1] in ['building', 'established']:
            return "ascending"
        else:
            return "variable"