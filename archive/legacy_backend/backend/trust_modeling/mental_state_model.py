...
class MentalStateModel:
    ...
    def _init_pattern_matchers(self):
        ...
        self.belief_patterns = {
            ...
            'confidence': re.compile(r'\b(i know|understand|got it|makes sense)\b', re.I),
            'trust': re.compile(r'\b(trust|believe|rely on|count on)\b', re.I),
            'distrust': re.compile(r'\b(don\'t trust|worried|concerned|suspicious)\b', re.I)
        }
        
        # Desire indicators
        self.desire_patterns = {
            'urgent': re.compile(r'\b(asap|urgent|quickly|right now|immediately)\b', re.I),
            'learning': re.compile(r'\b(learn|understand|explain|teach|how does)\b', re.I),
            'just_do_it': re.compile(r'\b(just|simply|only|don\'t care how)\b', re.I)
        }
        
        # Emotion indicators
        self.emotion_patterns = {
            'frustration': re.compile(r'\b(frustrated|annoyed|irritated|ugh|argh)\b', re.I),
            'happiness': re.compile(r'\b(great|awesome|perfect|thank|love)\b', re.I),
            'confusion': re.compile(r'\b(confused|lost|don\'t understand|huh|\?+)\b', re.I)
        }
        
    def update_from_interaction(self, interaction_id: str,
    ...
    def _get_user_context_from_skg(self) -> Dict:
        ...
        return {
            ...
            'success_rate': 1.0 - (error_stats['unresolved_errors'] / 
                                  max(1, error_stats['total_errors']))
        }
    ...
    def _calculate_learning_momentum(self, trajectory: List[Dict]) -> float:
        ...
        mastery_count = sum(1 for e in recent_events if e['type'] == 'mastery_milestone')
    ...
    def _determine_overall_state(self) -> str:
        ...
        if self.emotions.frustration_level > 0.7:
            return "frustrated"
        elif self.emotions.confidence_level > 0.7 and self.beliefs.self_efficacy > 0.7:
            return "confident"
        elif self.desires.learning_desire > 0.7:
            return "curious"
        elif self.desires.urgency_level > 0.7:
            return "urgent"
        elif self.emotions.engagement_level < 0.3:
            return "disengaged"
        else:
            return "neutral"
    ...
    def _determine_support_need(self) -> str:
        ...
        elif self.beliefs.understanding_level < 0.3:
            return "educational_support"
        elif self.desires.learning_desire > 0.7:
            return "detailed_explanation"
        elif self.desires.urgency_level > 0.7:
            return "quick_solution"
        elif self.desires.autonomy_preference > 0.7:
    ...
    def _get_emotional_tone(self) -> str:
        ...
        if self.emotions.frustration_level > 0.6:
            return "frustrated"
        elif self.emotions.confidence_level > 0.7:
            return "confident"
        elif self.emotions.engagement_level > 0.7:
            return "engaged"
        elif self.emotions.stress_indicators > 0.6:
            return "stressed"
        else:
            return "neutral"