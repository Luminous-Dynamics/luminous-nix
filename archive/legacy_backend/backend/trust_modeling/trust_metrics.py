...
class TrustMetrics:
    ...
    def calculate_trust_score(self, mental_state: Dict,
                            vulnerability_history: List[Dict],
                            repair_history: List[Dict]) -> float:
        """
        ...
        self._record_trust_metrics(overall_score, {
    ...
    def _calculate_benevolence(self, mental_state: Dict,
                             vulnerability_history: List[Dict]) -> float:
        """
        ...
        if vulnerability_history:
            recent_vulnerabilities = [
                v for v in vulnerability_history
    ...
    def _calculate_integrity(self, vulnerability_history: List[Dict],
                           repair_history: List[Dict]) -> float:
        """
    ...
    def _calculate_predictability(self) -> float:
        ...
        for response in recent_responses:
            ...
            pattern = {
                ...
                'has_explanation': 'because' in ai_response.lower() or 'since' in ai_response.lower(),
                'has_alternative': 'alternatively' in ai_response.lower() or 'instead' in ai_response.lower(),
    ...
    def _apply_trust_momentum(self, new_score: float) -> float:
        ...
        adjusted_score = (
            momentum_weight * recent_score +
            (1 - momentum_weight) * new_score
        )
    ...
    def _assess_vulnerability_appropriateness(self, 
                                           vulnerability_history: List[Dict]) -> float:
    ...
    def _detect_tone(self, text: str) -> str:
        ...
        if any(word in text_lower for word in ['apologize', 'sorry', 'mistake']):
            return 'apologetic'
        elif any(word in text_lower for word in ['great', 'excellent', 'perfect']):
            return 'enthusiastic'
        elif '?' in text:
            return 'questioning'
        elif any(word in text_lower for word in ['let me', 'i can', "i'll"]):
            return 'helpful'
        else:
            return 'neutral'
            
    def _record_trust_metrics(self, overall_score: float,
        ...
        for component, scores in self.component_scores.items():
            if scores:
                ...
                analysis[component] = {
                    ...
                    'stability': 1 - np.std(recent_scores) if len(recent_scores) > 1 else 1.0
                }
            else:
                analysis[component] = {
                    'current': 0.5,
