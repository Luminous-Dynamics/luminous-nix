...
from .response_adapter import create_simple_response, adapt_enhanced_response
...
class NixForHumanityBackend:
    ...
    def _init_research_components(self):
        ...
        try:
            ...
            skg_path = os.getenv('NIX_HUMANITY_SKG_PATH', './nix_humanity_skg.db')
            ...
            if os.getenv('NIX_HUMANITY_ACTIVITY_TRACKING', 'false').lower() == 'true':
            ...
            if interaction_id and self.skg:
                ...
                if self.trust_engine:
                    ...
                    if trust_update.get('vulnerability_action'):
                        ...
                        response.data['trust_building'] = trust_update['vulnerability_action']
                ...
                if self.metrics_collector:
                    metrics = self.metrics_collector.collect_current_metrics({
                        ...
                        'interruptions': request.context.get('interruption_count', 0),
    ...
    async def _learn(self, request: Request, response: Response):
        ...
        try:
            ...
            if self.skg:
                ...
                if hasattr(request.context, 'user_feedback'):
                    self.skg.phenomenological.record_experience({
                        ...
                        'intent': response.intent.type.value if response.intent else None
                    })
                ...
                if response.success:
                    self.skg.metacognitive.record_meta_learning({
                        ...
                        'context': {
                            'intent': response.intent.type.value if response.intent else None,
                            'approach': 'traditional' if not self.consciousness_guard else 'awareness-based'
                        },
            ...
            if os.getenv('DEBUG'):
                print(f"Learning from: {request_text} -> {response.intent.type.value if response.intent else 'unknown'}")
