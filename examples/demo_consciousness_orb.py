...
def render_info(state: str, emotion: str, phase: int):
    ...
    info = Text()
    info.append("AI State: ", style="dim")
    info.append(f"{state}\n", style="bold green")
    info.append("Emotion: ", style="dim")
    info.append(f"{emotion}\n", style="bold magenta")
    info.append("Animation Phase: ", style="dim")
    ...
    with Live(layout, console=console, refresh_per_second=30) as live:
        for state, emotion, breathing_rate, duration in sequences:
            ...
            while time.time() - start_time < duration:
                ...
                layout["info"].update(
                    render_info(state.value, emotion.value[0], int(orb.phase))
                )
...
def main():
    ...
    console.print(
        "This demonstrates the living, breathing visual presence of our AI partner."
    )
    console.print(
        "Watch as the orb transitions through different states and emotions...\n"
    )
    ...
    try:
        ...
        console.print(
            "• [cyan]Living presence[/cyan] through smooth breathing animations"
        )
        ...
        console.print(
            "This is consciousness-first design - technology that breathes with life! 🌊\n"
        )
