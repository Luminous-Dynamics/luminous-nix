...
class NativeTUIDemo(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(id="container"):
            ...
            with Vertical():
                ...
                yield Button(
                    "🔍 System Info (INSTANT!)", id="system-info", variant="primary"
                )
                yield Button(
                    "📦 Search Firefox (10x faster)",
                    id="search-firefox",
                    variant="primary",
                )
                yield Button(
                    "🗑️ Check Garbage Collection", id="check-gc", variant="primary"
                )
                yield Button(
                    "🔧 Show Config Options (INSTANT!)",
                    id="show-options",
                    variant="primary",
                )
    ...
    async def run_native_operation(
        self, op_type: NativeOperationType, packages: list = None
    ):
        ...
        try:
            result = await self.native_ops.execute_native_operation(
                op_type, packages=packages
            )
            duration = (time.time() - start) * 1000

            if result.success:
                # Format result based on operation
                if op_type == NativeOperationType.LIST_GENERATIONS and result.data.get(
                    "generations"
                ):
                ...
                self.update_result(op_type.value, f"✅ {msg}", f"{duration:.1f}ms")
            else:
                self.update_result(
                    op_type.value, f"❌ {result.message}", f"{duration:.1f}ms"
                )
        except Exception as e:
            self.update_result(op_type.value, f"❌ Error: {str(e)}", "N/A")
