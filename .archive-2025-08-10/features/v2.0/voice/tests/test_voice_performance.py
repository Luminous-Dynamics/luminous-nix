...
class TestVoicePerformanceBenchmarks:
    ...
    async def test_model_loading_performance(
        self, voice_interface_optimized, performance_monitor
    ):
        ...
        for persona in personas_to_test:
            ...
            passed, issues = self.perf_test.validate_performance(
                result, benchmark, persona
            )

            assert (
                passed
            ), f"Model loading performance failed for {persona}: {', '.join(issues)}"
            assert (
                result.success
            ), f"Model loading failed for {persona}: {result.error_message}"

    @pytest.mark.performance
    @pytest.mark.asyncio
    async def test_speech_transcription_performance(
        self, voice_interface_optimized, performance_monitor
    ):
        ...
        async def transcribe_speech():
            ...
            with patch.object(
                voice_interface_optimized, "_transcribe_audio", new_callable=AsyncMock
            ) as mock_transcribe:
                mock_transcribe.return_value = "install firefox"

                # Simulate processing time based on audio length
                await asyncio.sleep(0.05)  # Simulate fast transcription
                return await voice_interface_optimized._transcribe_audio(
                    b"mock_audio_data"
                )
        ...
        for persona in personas_to_test:
            ...
            assert (
                passed
            ), f"Speech transcription performance failed for {persona}: {', '.join(issues)}"
            assert (
                result.success
            ), f"Speech transcription failed for {persona}: {result.error_message}"

    @pytest.mark.performance
    @pytest.mark.asyncio
    async def test_intent_processing_performance(
        self, voice_interface_optimized, performance_monitor
    ):
        ...
        for persona in personas_to_test:
            ...
            assert (
                passed
            ), f"Intent processing performance failed for {persona}: {', '.join(issues)}"
            assert (
                result.success
            ), f"Intent processing failed for {persona}: {result.error_message}"

    @pytest.mark.performance
    @pytest.mark.asyncio
    async def test_speech_synthesis_performance(
        self, voice_interface_optimized, performance_monitor
    ):
        ...
        async def synthesize_speech():
            ...
            with patch.object(
                voice_interface_optimized, "_synthesize_speech", new_callable=AsyncMock
            ) as mock_synthesize:
                mock_synthesize.return_value = b"mock_audio_data"

                # Simulate TTS processing time
                await asyncio.sleep(0.08)  # Simulate TTS generation
                return await voice_interface_optimized._synthesize_speech(
                    "Installing Firefox for you!"
                )
        ...
        for persona in personas_to_test:
            ...
            assert (
                passed
            ), f"Speech synthesis performance failed for {persona}: {', '.join(issues)}"
            assert (
                result.success
            ), f"Speech synthesis failed for {persona}: {result.error_message}"

    @pytest.mark.performance
    @pytest.mark.asyncio
    async def test_end_to_end_voice_performance(
        self, voice_interface_optimized, performance_monitor
    ):
        ...
        async def end_to_end_voice():
            ...
            with (
                ...
                return await voice_interface_optimized.process_voice_input(
                    b"mock_audio_input"
                )
        ...
        for persona in personas_to_test:
            ...
            assert (
                passed
            ), f"End-to-end voice performance failed for {persona}: {', '.join(issues)}"
            assert (
                result.success
            ), f"End-to-end voice failed for {persona}: {result.error_message}"
    ...
    async def test_concurrent_voice_requests(
        ...
        async def simulate_voice_request(request_id: int):
            ...
            with patch.object(
                voice_interface_optimized, "process_voice_input", new_callable=AsyncMock
            ) as mock_process:
        ...
        assert (
            duration_ms < 3000
        ), f"Concurrent processing took {duration_ms}ms, too slow"

        # Verify memory usage stays reasonable
        assert (
            memory_used < 500
        ), f"Concurrent processing used {memory_used}MB, too much"
        ...
        async def repeated_voice_operations():
            ...
            for i in range(10):  # 10 iterations
                with patch.object(
                    ...
                    await voice_interface_optimized.process_voice_input(
                        f"test command {i}"
                    )
        ...
        for benchmark_name, benchmark in VoicePerformanceTest.BENCHMARKS.items():
            for persona in required_personas:
                if persona == "maya_adhd":
                    ...
                    assert (
                        persona in benchmark.persona_requirements
                    ...
                    assert (
                        maya_limit <= benchmark.max_duration_ms
...
class TestVoiceScalabilityBenchmarks:
    ...
    async def test_model_cache_effectiveness(self, voice_interface_optimized):
        ...
        start_time = time.perf_counter()
        with patch.object(
            voice_interface_optimized.model_manager,
            "get_whisper_model",
            new_callable=AsyncMock,
        ) as mock_whisper:
        ...
        improvement_ratio = (
            cold_load_time / warm_load_time if warm_load_time > 0 else float("inf")
        )
        assert (
            improvement_ratio > 2.0
        ), f"Cache not effective: {improvement_ratio:.1f}x improvement"
    ...
    async def test_degraded_performance_under_load(self, voice_interface_optimized):
        ...
        try:
            ...
            assert (
                duration_ms < 10000
            ), f"Voice processing took {duration_ms}ms under load, too slow"
