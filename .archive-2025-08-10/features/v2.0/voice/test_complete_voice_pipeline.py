    ...
    try:
        ...
        async with websockets.connect(uri) as websocket:
            ...
            await websocket.send(
                json.dumps({"type": "text", "text": "I need to check my email"})
            )
            ...
            await websocket.send(
                json.dumps(
                    {
                        "type": "settings",
                        "settings": {"voiceSpeed": 0.8, "voiceVolume": 0.9},
                    }
                )
            )
    ...
    try:
        async with websockets.connect("ws://localhost:8765") as websocket:
            ...
            for command, expected_intent in test_commands:
                ...
                # Send command
                await websocket.send(json.dumps({"type": "text", "text": command}))
...
def test_frontend_compatibility():
    ...
    required_files = ["index.html", "styles.css", "voice-interface.js"]
