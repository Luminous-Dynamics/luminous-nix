...
class VoiceConnection:
    ...
    def __init__(
        ...
        on_metrics_update: Callable[[VoiceMetrics], None] | None = None,
    ):
        """
    ...
    def _simulate_audio_chunk(self, speaking: bool = False) -> np.ndarray:
        ...
        if speaking:
            ...
            signal = (
                ...
                + 0.2 * np.sin(2 * np.pi * 400 * t)  # Fundamental
                + 0.1 * np.sin(2 * np.pi * 800 * t)  # Harmonic
                + 0.05 * np.random.randn(self.chunk_size)  # Harmonic  # Noise
            )
            # Add envelope
...
class VoiceIntegration:
    ...
    def _handle_voice_command(self, text: str) -> None:
        ...
        asyncio.create_task(
            self.tui.on_input_submitted(type("Event", (), {"value": text})())
        )
    ...
    def _update_voice_metrics(self, metrics: VoiceMetrics) -> None:
        ...
        self.tui.orb.set_voice_activity(
            metrics.amplitude > 0.1, metrics.amplitude, metrics.frequency
        )
...
async def demo_voice_connection():
    ...
    def on_metrics(metrics: VoiceMetrics):
        ...
        print(
            f"\r🎤 {waveform} | Amp: {metrics.amplitude:.2f} | Freq: {metrics.frequency:.0f}Hz",
            end="",
        )
