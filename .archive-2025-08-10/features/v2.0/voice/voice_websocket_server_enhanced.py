...
class VoiceWebSocketServer:
    ...
    async def register(self, websocket):
        ...
        await websocket.send(
            json.dumps(
                {"type": "welcome", "message": "Voice interface connected and ready!"}
            )
        )
    ...
    async def handle_audio(self, websocket, data):
        ...
        try:
            ...
            await websocket.send(
                json.dumps(
                    {"type": "status", "message": "Processing your voice command..."}
                )
            )
            ...
            try:
                ...
                (
                    adapted_response,
                    _,
                ) = self.response_formatter.adapt_response_with_dimensions(
                    response_text, self.simple_dimensions
                )

                # Generate speech
                audio_path = self.voice_interface.speak(
                    adapted_response, return_path=True
                )
    ...
    async def handle_settings(self, websocket, data):
        ...
        await websocket.send(
            json.dumps(
                {"type": "settings_updated", "message": "Settings updated successfully"}
            )
        )
