...
from luminous_nix.ai.xai_engine import (
    CausalFactor,
    ConfidenceLevel,
    Explanation,
    ExplanationLevel,
...
class TestXAIEngineCore(unittest.TestCase):
    ...
    def test_simple_explanation_generation_beginner(self):
        ...
        factors = [
            CausalFactor("keyword_match", 0.8, "positive", "found 'install' keyword", 1)
        ]

        explanation = self.engine._generate_simple_explanation(
            "intent_recognition", "install_package", factors, "beginner"
        )
    ...
    def test_simple_explanation_generation_expert(self):
        ...
        explanation = self.engine._generate_simple_explanation(
            "intent_recognition", "install_package", factors, "expert"
        )
    ...
    def test_detailed_explanation_generation(self):
        ...
        factors = [
            CausalFactor(
                "keyword_match", 0.8, "positive", "found 'install' keyword", 2
            ),
            CausalFactor(
                "user_history", 0.6, "positive", "user often installs packages", 5
            ),
        ...
        explanation = self.engine._generate_detailed_explanation(
            "intent_recognition", "install_package", factors, context
        )
    ...
    def test_technical_explanation_generation(self):
        ...
        factors = [
            CausalFactor("keyword_match", 0.8, "positive", "keyword analysis", 1)
        ]
    ...
    def test_alternatives_finding_package_selection(self):
        ...
        alternatives = self.engine._find_alternatives(
            "package_selection", "firefox", {"category": "browser"}
        )
    ...
    def test_database_decision_storage(self):
        ...
        explanation = self.engine.explain_decision(
            "test_decision", "test_value", context, factors
        )
        ...
        c.execute(
            "SELECT * FROM causal_factors WHERE decision_id = ?", (decision_row[0],)
        )
    ...
    def test_explanation_history_retrieval(self):
        ...
        for i in range(3):
            self.engine.explain_decision(
                f"test_type_{i}", f"value_{i}", context, factors
            )
        ...
        self.assertEqual(
            history[0]["decision_type"], "test_type_2"
        )  # Most recent first
    ...
    def test_explanation_feedback_recording(self):
        ...
        c = conn.cursor()
        c.execute(
            "SELECT helpful_rating, user_feedback FROM user_explanations WHERE decision_id = ?",
            (decision_id,),
        )
...
class TestAnalyzeIntentRecognition(unittest.TestCase):
    ...
    def test_analyze_intent_recognition_with_history(self):
        ...
        context = {"user_history": {"similar_patterns": 5}}
        ...
        history_factor = next(
            f for f in explanation.causal_factors if f.name == "user_history"
        )
    ...
    def test_analyze_intent_recognition_low_confidence(self):
        ...
        low_conf_factor = next(
            f for f in explanation.causal_factors if f.name == "low_confidence"
        )
...
class TestEdgeCases(unittest.TestCase):
    ...
    def test_invalid_decision_type(self):
        ...
        explanation = self.engine.explain_decision(
            "unknown_decision_type", "value", {}, factors
        )
    ...
    def test_very_long_input_handling(self):
        ...
        explanation = self.engine.explain_decision(
            "test", "value", long_context, factors
        )
        self.assertIsInstance(explanation, Explanation)
