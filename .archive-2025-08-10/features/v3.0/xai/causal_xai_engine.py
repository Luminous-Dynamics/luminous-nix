...
try:
    ...
    logging.warning(
        "SHAP not available. Feature importance will use simple heuristics."
    )
...
class CausalXAIEngine:
    ...
    def _load_causal_patterns(self) -> dict[str, dict]:
        ...
        return {
            ...
            "search_package": {
                "causes": {"exploration": 0.5, "specific_need": 0.3, "comparison": 0.2},
                "effects": {"knowledge_gain": 0.9, "decision_quality": 0.7},
            },
        }
    ...
    def explain_intent(
        ...
        if depth == ExplanationDepth.SIMPLE:
            ...
            explanation = self._generate_detailed_explanation(
                action, package, causal_factors, context
            )
            ...
            explanation = self._generate_standard_explanation(
                action, package, confidence, causal_factors
            )
    ...
    def _analyze_causal_factors(self, action: str, context: dict) -> dict[str, float]:
        ...
        for cause, base_weight in causes.items():
            ...
            elif cause == "security" and context.get(
    ...
    def _generate_simple_explanation(
        self, action: str, package: str, confidence: float
    ) -> CausalExplanation:
    ...
    def _generate_standard_explanation(
        self,
        action: str,
        package: str,
        confidence: float,
        ...
        primary_cause = (
            max(causal_factors.items(), key=lambda x: x[1])[0]
            if causal_factors
            else "unknown"
        )
        ...
        explanations = {
            "install_package": {
                ...
                "alternatives": [
                    "Install system-wide in configuration.nix",
            ...
            "remove_package": {
                ...
                "risks": [
                    ...
                    "Dependent packages might be affected",
                ],
    ...
    def _build_why_narrative(
        self, action: str, package: str, primary_cause: str
    ) -> str:
        ...
        narratives = {
            ...
            "search": {
                ...
                "specific_need": "You need to find the exact package name",
                "comparison": f"You want to compare different {package} options",
            },
        }

        action_narratives = narratives.get(action, {})
        return action_narratives.get(
            primary_cause, "This action will help with your request"
        )

    def _generate_detailed_explanation(
        self, action: str, package: str, causal_factors: dict[str, float], context: dict
    ) -> CausalExplanation:
        """Generate detailed explanation with examples"""

        # Get standard explanation first
        base = self._generate_standard_explanation(
            action,
            package,
    ...
    def _generate_technical_explanation(
        ...
        base = self._generate_detailed_explanation(
            action, package, causal_factors, context
        )
...
Time estimate: 1-2 seconds""",
    ...
    def _generate_detailed_risks(
        self, action: str, package: str, context: dict
    ) -> list[str]:
        ...
        risks = {
            ...
            "search_package": ["No significant risks"],
        }

        return risks.get(action, ["Unknown operation risks"])

    def _generate_detailed_benefits(
        self, action: str, package: str, context: dict
    ) -> list[str]:
    ...
    def format_explanation_for_display(
        ...
        if self.user_preferences["include_confidence"]:
            ...
            output.append(
                f"🎯 Confidence: {confidence_level.value[2]} ({explanation.confidence:.0%})"
            )
            output.append("")
        ...
        if depth in [
            ...
            if explanation.causal_factors:
                ...
                sorted_factors = sorted(
                    explanation.causal_factors.items(), key=lambda x: x[1], reverse=True
                )
                for factor, weight in sorted_factors[:3]:  # Top 3 factors
                    output.append(
                        f"  • {factor.replace('_', ' ').title()}: {weight:.0%}"
                    )
    ...
    def learn_from_outcome(
        self, intent: dict, outcome: dict, user_feedback: dict | None = None
    ):
    ...
    def get_explanation_summary(self) -> dict[str, Any]:
        ...
        successful = sum(
            1
            for e in self.explanation_cache.values()
        ...
        return {
            ...
            "success_rate": (
                successful / total_explanations if total_explanations > 0 else 0
            ),
            "preferences": self.user_preferences,
            "available_features": {"dowhy": DOWHY_AVAILABLE, "shap": SHAP_AVAILABLE},
        }
