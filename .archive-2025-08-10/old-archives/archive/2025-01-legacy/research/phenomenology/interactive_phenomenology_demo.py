...
class InteractivePhenomenologyDemo:
    ...
    def _get_state_indicator(self):
        ...
        if qualia.cognitive_load > 0.7:
            return f"{Colors.RED}◈{Colors.ENDC}"  # Overloaded
        if qualia.learning_momentum > 0.6:
            return f"{Colors.BLUE}📚{Colors.ENDC}"  # Learning
        if hasattr(qualia, "frustration_level") and qualia.frustration_level > 0.5:
    ...
    def _process_query(self, query):
        ...
        if query.lower() == "status":
            self._show_status()
            return
        if query.lower().startswith("simulate "):
        ...
        temporal_state = TemporalSystemState(
            timestamp=datetime.now(), state=system_state, **activity_data
        )
    ...
    def _generate_response(
        self, query: str, qualia: DynamicQualiaVector
    ) -> dict[str, Any]:
    ...
    def _learning_style(
        ...
        return {
            ...
            "example": "For example: nix flake init",
            "success": True,
            "educational": True,
        }

    def _frustrated_style(
        self, query: str, qualia: DynamicQualiaVector
    ) -> dict[str, Any]:
    ...
    def _neutral_style(self, query: str, qualia: DynamicQualiaVector) -> dict[str, Any]:
        ...
        return {"text": f"Processing: {query}", "success": True, "standard": True}
    ...
    def _update_metrics(self, qualia: DynamicQualiaVector):
        """Update consciousness metrics"""
        self.metrics_collector.update_flow_state(
            qualia.flow, stable=qualia.stability > 0.7
        )
    ...
    def _show_status(self):
        ...
        print(
            f"Cog. Load:   {self._bar(qualia.cognitive_load)} {qualia.cognitive_load:.0%}"
        )
        print(
            f"Learning:    {self._bar(qualia.learning_momentum)} {qualia.learning_momentum:.0%}"
        )
    ...
    def _print_session_summary(self):
        ...
        if self.session.interactions:
            ...
            for state, count in sorted(
                ...
                print(
                    f"  {state}: {count} ({count/len(self.session.interactions):.0%})"
                )
