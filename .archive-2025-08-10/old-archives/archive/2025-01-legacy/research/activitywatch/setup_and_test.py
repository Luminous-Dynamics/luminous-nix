...
class ActivityWatchExplorer:
        ...
        try:
            ...
            for event in events:
                ...
                if app not in app_usage:
                    app_usage[app] = {"count": 0, "total_duration": 0, "titles": set()}
            ...
            analysis = {
                ...
                "app_usage": app_usage,
                "most_used_app": (
                    max(app_usage.items(), key=lambda x: x[1]["total_duration"])[0]
                    if app_usage
                    else None
                ),
            }
    ...
    def create_test_bucket(self) -> bool:
        ...
        try:
            # Create bucket
            self.client.create_bucket(bucket_id, event_type="nix-command", queued=True)
