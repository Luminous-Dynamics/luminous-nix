...
class ResearchProgressTracker:
    ...
    def update_phase(self, item_id: str, new_phase: ResearchPhase):
        ...
        self._log_event(
            item_id, "phase_change", f"{old_phase.value} → {new_phase.value}"
        )
    ...
    def get_ready_items(self) -> list[ResearchItem]:
        ...
        for item in self.items.values():
            if item.phase == ResearchPhase.NOT_STARTED and not item.is_blocked(
                completed
            ):
                ready.append(item)
    ...
    def get_in_progress(self) -> list[ResearchItem]:
        ...
        return [
            item for item in self.items.values() if item.phase in in_progress_phases
        ]
    ...
    def generate_progress_report(self) -> dict:
        ...
        complete = sum(
            1 for item in self.items.values() if item.phase == ResearchPhase.COMPLETE
        )
        ...
        report = {
            "summary": {
                ...
                "weighted_progress": (
                    (weighted_progress / total_weight) * 100 if total_weight > 0 else 0
                ),
            },
            "by_category": {},
            "by_phase": {},
            ...
            "blockers": [
                (item.name, blocking) for item, blocking in self.get_blocked_items()
            ],
        }
    ...
    def visualize_progress(self, output_path: str = "research_progress.png"):
        ...
        progress = [
            report["by_category"][cat]["average_progress"] for cat in categories
        ]
        ...
        if in_progress:
            ...
            # Add progress text
            for i, (item, prog) in enumerate(zip(in_progress, progress, strict=False)):
    ...
    def print_summary(self):
        ...
        for category, data in report["by_category"].items():
            print(
                f"  {category}: {data['average_progress']:.1f}% ({data['completed']}/{data['total']})"
            )
    ...
    def save_item(self, item_id: str):
        ...
        """,
            (
                ...
                json.dumps(data["milestones"]),
                json.dumps(data["completed_milestones"]),
                data["notes"],
                data["start_date"],
                data["target_date"],
...
if __name__ == "__main__":
    ...
    tracker.complete_milestone(
        "activitywatch_integration", "Install and configure ActivityWatch"
    )
    ...
    if ready:
        ...
        for item in ready:
            print(
                f"  • {item.name} (Priority: {item.priority.name}, Impact: {item.impact}/5)"
            )
