...
class MetricsCollector:
    ...
    def __init__(self, db_path: Path | None = None, retention_days: int = 7):
    ...
    def _init_db(self):
        ...
        with sqlite3.connect(str(self.db_path)) as conn:
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_health_name_timestamp ON health_checks(name, timestamp)"
            )
    ...
    def record_operation(
        ...
        self.record_metric(
            ...
            duration,
            tags={"success": str(success).lower(), "operation": operation_type},
    ...
    def get_metrics_summary(
        ...
        with self._lock:
            for name, data in self.aggregated_metrics.items():
                recent_values = [
                    v
                    for v, t in zip(data["values"], data["timestamps"], strict=False)
                    if t >= cutoff_time
                ]
    ...
    def export_metrics(
        self, format: str = "json", time_range: timedelta | None = None
    ) -> str:
    ...
    def _format_prometheus(self, metrics: list[dict[str, Any]]) -> str:
        ...
        for metric in metrics:
            ...
            if tags:
                lines.append(
                    f"{name}{{{tags}}} {metric['value']} {int(metric['timestamp'] * 1000)}"
                )
            else:
                lines.append(
                    f"{name} {metric['value']} {int(metric['timestamp'] * 1000)}"
                )
    ...
    def _cleanup_old_data(self):
        ...
        with sqlite3.connect(str(self.db_path)) as conn:
            ...
            conn.execute(
                "DELETE FROM health_checks WHERE timestamp < ?", (cutoff_time,)
            )
    ...
    def _check_alerts(self, metric: Metric):
        ...
        if metric.name == "operation.update_system.duration" and metric.value > 60:
            self._trigger_alert(
                f"System update took {metric.value:.1f}s (threshold: 60s)"
            )
...
class timed_operation:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        record_operation(self.operation_type, duration, self.success, error=self.error)
...
if __name__ == "__main__":
    ...
    for i in range(10):
        ...
        record_metric(
            "cache.test.hit" if random.random() < 0.7 else "cache.test.miss", 1.0
        )
