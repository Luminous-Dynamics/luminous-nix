...
class FlowStateDetector:
    ...
    def _update_behavior_buffer(self, interaction_data: dict):
        ...
        self.behavior_buffer = [
            b for b in self.behavior_buffer if b["timestamp"] > cutoff
        ]
    ...
    def _calculate_flow_indicators(self) -> FlowIndicators:
        ...
        session_duration = (
            self.behavior_buffer[-1]["timestamp"] - self.behavior_buffer[0]["timestamp"]
        ).seconds / 60
        ...
        recent_mastery = sum(
            1
            for event in learning_trajectory[-10:]
        ...
        self_consciousness_loss = (
            max(0, 1 - cognitive_load) if cognitive_load > 0.3 else 0.5
        )
    ...
    def _calculate_flow_level(self, indicators: FlowIndicators) -> float:
        ...
        flow_level = sum(weights[key] * value for key, value in indicator_dict.items())
    ...
    def _recommend_flow_protection(
        self, flow_state: str, indicators: FlowIndicators
    ) -> list[dict]:
    ...
    def _calculate_interruption_window(self) -> dict | None:
        ...
        profile = self.task_flow_profiles.get(
            current_task, {"optimal_duration": 60, "warmup_time": 10}
        )
        ...
        if flow_duration > profile["optimal_duration"] - 10:
            return {
    ...
    def _record_flow_assessment(self, assessment: dict):
        ...
        """,
            (assessment_id, json.dumps(assessment)),
        )
    ...
    def get_flow_report(self) -> dict:
        ...
        total_flow_time = sum(
            session["duration_minutes"] for session in self.flow_history
        )

        avg_flow_duration = (
            total_flow_time / len(self.flow_history) if self.flow_history else 0
        )

        # Get flow quality distribution
        flow_quality = {
            "deep_flow_sessions": sum(
                1 for s in self.flow_history if s["peak_level"] >= 0.8
            ),
            "flow_sessions": sum(
                1 for s in self.flow_history if 0.6 <= s["peak_level"] < 0.8
            ),
            "near_flow_sessions": sum(
                1 for s in self.flow_history if s["peak_level"] < 0.6
            ),
        }
    ...
    def _identify_optimal_flow_conditions(self) -> dict:
        ...
        successful_flows = [s for s in self.flow_history if s["peak_level"] >= 0.7]
        ...
        return {
            "average_duration": np.mean(
                [s["duration_minutes"] for s in successful_flows]
            ),
    ...
    def suggest_flow_enhancement(self) -> dict | None:
