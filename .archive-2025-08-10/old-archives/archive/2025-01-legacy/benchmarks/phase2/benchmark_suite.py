...
from src.luminous_nix.ai.context_manager import EnhancedContextManager
from src.luminous_nix.core.nlp_engine import NLPEngine
from src.luminous_nix.tui.persona_system import PersonaSystem
...
class PerformanceBenchmark:
    ...
    def benchmark_nlp_engine(self):
        ...
        for input_text in test_inputs:
            with self.measure_performance(
                f"NLP: {input_text[:30]}...", "NLP Processing"
            ) as measure:
    ...
    def benchmark_xai_engine(self):
        ...
        for intent_type, params in test_intents:
            for persona in personas:
                with self.measure_performance(
                    f"XAI: {intent_type} for {persona}", "XAI Generation"
                ) as measure:
                    measure(
                        lambda: self.xai_engine.explain(
                            intent_type, params, persona=persona
                        )
                    )
    ...
    def benchmark_learning_system(self):
        ...
        with self.measure_performance(
            "Learning: Record preference", "Learning System"
        ) as measure:
            measure(
                lambda: self.learning_system.record_preference(
                    "editor", "neovim", "Maya"
                )
            )
        ...
        with self.measure_performance(
            "Learning: Extract patterns", "Learning System"
        ) as measure:
            measure(lambda: self.learning_system.extract_patterns("Maya"))

        # Test adaptation
        with self.measure_performance(
            "Learning: Adapt response", "Learning System"
        ) as measure:
            measure(
                lambda: self.learning_system.adapt_response(
    ...
    def benchmark_persona_adaptation(self):
        ...
        for persona_name in personas:
            with self.measure_performance(
                f"Persona: Switch to {persona_name}", "Persona System"
            ) as measure:
                measure(lambda: self.persona_system.set_active_persona(persona_name))

            with self.measure_performance(
                f"Persona: Adapt UI for {persona_name}", "Persona System"
            ) as measure:
    ...
    def benchmark_context_management(self):
        ...
        # Add context
        with self.measure_performance(
            "Context: Add interaction", "Context Management"
        ) as measure:
            measure(
                lambda: self.context_manager.add_context(
                    "user asked about firefox", "Maya"
                )
            )

        # Retrieve context
        with self.measure_performance(
            "Context: Get relevant context", "Context Management"
        ) as measure:
            measure(
                lambda: self.context_manager.get_relevant_context("install it", "Maya")
            )
    ...
    def benchmark_integration(self):
        ...
        for input_text, persona in test_flows:
            with self.measure_performance(
                f"Integration: {input_text} ({persona})", "Full Pipeline"
            ) as measure:
    ...
    def run_all_benchmarks(self):
        ...
        print(
            f"   Average response: {statistics.mean([r.avg_time_ms for r in self.results]):.2f}ms"
        )
