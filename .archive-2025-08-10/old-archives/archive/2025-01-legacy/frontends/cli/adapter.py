...
from nix_humanity.core import Context, NixForHumanityBackend, Request, Response
...
For more help: ask-nix --docs
        ...
        # Main query argument
        parser.add_argument(
            "query", nargs="*", help="Natural language query or command"
        )
        ...
        # Input/output modes
        parser.add_argument(
            "--voice", "-v", action="store_true", help="Enable voice input/output"
        )
        ...
        parser.add_argument(
            "--no-cache", action="store_true", help="Disable cache for fresh results"
        )
        parser.add_argument(
        ...
        # Utilities
        parser.add_argument(
            "--summary", action="store_true", help="Show system learning summary"
        )
        ...
        # Plugin support
        parser.add_argument("--plugin", help="Use specific plugin")
        parser.add_argument(
    ...
    def build_request(self, args: argparse.Namespace) -> Request:
        ...
        return Request(query=query, context=context)
    ...
    def collect_feedback(self, query: str, response: Response):
        ...
        try:
            ...
            if helpful == "n":
                improved = input(
                    "What would have been better? (or Enter to skip): "
                ).strip()
                if improved:
                    feedback_data["improved_response"] = improved

            # Send feedback to backend
            feedback_context = Context(frontend="cli", session_id=self.session_id)
            feedback_context.user_preferences["feedback"] = feedback_data

            feedback_request = Request(
                query="record feedback", context=feedback_context
            )
