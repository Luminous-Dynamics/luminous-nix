...
class NixForHumanityDemo:
    ...
    async def demo_performance(self):
        ...
        operations = [
            ...
            (
                "Search Packages",
                NixOperation(type=OperationType.SEARCH, packages=["python"]),
            ),
    ...
    async def demo_monitoring(self):
        ...
        for metric, data in list(metrics_summary.items())[:5]:
            print(
                f"   • {metric}: {data.get('mean', 0):.2f} (count: {data.get('count', 0)})"
            )
        ...
        for op, stats in operation_stats.items():
            if stats["total"] > 0:
                print(
                    f"   • {op}: {stats['success_rate']:.1%} success rate, "
    ...
    async def demo_progress_tracking(self):
        ...
        if self.progress_history:
            ...
            print(
                f"   • Duration: {self.progress_history[-1]['timestamp'] - self.progress_history[0]['timestamp']:.2f}s"
            )
            ...
            milestones = [
                ...
                if p["progress"] in [0.0, 0.25, 0.5, 0.75, 1.0]
            ]
            if milestones:
    ...
    async def run_complete_demo(self):
        ...
        print("=" * 60)
        print(
            "Demonstrating all enhanced features of the native Python-Nix integration"
        )

        # Check if we have proper permissions
        if os.geteuid() != 0 and not os.environ.get("LUMINOUS_NIX_ALLOW_UNPRIVILEGED"):
            print(
                "\n⚠️  Running in unprivileged mode. Some operations will be limited."
            )
            print("   Set LUMINOUS_NIX_ALLOW_UNPRIVILEGED=true for full demo")
        ...
        print(
            "   3. Explore the enhanced backend: python3 enhanced_native_nix_backend.py"
        )
        print(
            "   4. Read the documentation: docs/02-ARCHITECTURE/10-NATIVE-PYTHON-NIX-INTEGRATION.md"
        )
...
async def main():
    ...
    os.environ["LUMINOUS_NIX_ALLOW_UNPRIVILEGED"] = "true"
