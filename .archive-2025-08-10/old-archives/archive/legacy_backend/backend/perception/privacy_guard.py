...
class PrivacyGuard:
    ...
    def _sanitize_window_title(self, title: str) -> str:
        ...
        if self.policy.level == PrivacyLevel.BALANCED:
    ...
    def _generalize_title(self, title: str) -> str:
        ...
        if any(
            browser in title.lower()
            for browser in ["firefox", "chrome", "safari", "edge"]
        ):
            return "Web Browser"
        if any(
            editor in title.lower() for editor in ["code", "vim", "emacs", "sublime"]
        ):
            return "Code Editor"
        if any(term in title.lower() for term in ["terminal", "console", "shell"]):
            return "Terminal"
        if any(chat in title.lower() for chat in ["slack", "discord", "teams"]):
    ...
    def check_data_retention(self) -> dict[str, int]:
        ...
        retention_cutoff = datetime.now() - timedelta(
            days=self.policy.data_retention_days
        )
        anonymize_cutoff = datetime.now() - timedelta(
            days=self.policy.anonymize_after_days
        )
        ...
        """,
            (anonymize_cutoff.isoformat(), retention_cutoff.isoformat()),
        ).fetchone()[0]
    ...
    def enforce_data_retention(self) -> dict[str, int]:
        ...
        self.logger.info(
            f"Data retention enforced: {deleted} deleted, {anonymized} anonymized"
        )
    ...
    def _generate_privacy_recommendations(self, factors: dict[str, float]) -> list[str]:
        ...
        if factors["policy_strictness"] < 0.6:
            recommendations.append(
                "Consider increasing privacy level for better protection"
            )

        if factors["violation_rate"] < 0.8:
            recommendations.append(
                "Review recent privacy violations and adjust filters"
            )
        ...
        if not recommendations:
            recommendations.append(
                "Privacy health is excellent - keep up the good practices!"
            )
