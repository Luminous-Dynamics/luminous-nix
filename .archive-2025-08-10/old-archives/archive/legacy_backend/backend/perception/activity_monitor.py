...
class ActivityMonitor:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(f"{self.aw_url}/0/info") as response:
                    if response.status == 200:
                        ...
                        self.logger.info(
                            f"Connected to ActivityWatch {info.get('version', 'unknown')}"
                        )
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(f"{self.aw_url}/0/buckets") as response:
                if response.status == 200:
                    ...
                    self.logger.info(
                        f"Found buckets - Window: {self.window_bucket}, AFK: {self.afk_bucket}"
                    )

    async def get_current_activity(self) -> ActivitySnapshot | None:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                snapshot = ActivitySnapshot(
                    ...
                    window_title=self._sanitize_window_title(
    ...
    async def _get_afk_status(self, session: aiohttp.ClientSession) -> dict:
        ...
        try:
            ...
            query = {
                "timeperiods": [
                    f"{(datetime.now() - timedelta(minutes=5)).isoformat()}/{datetime.now().isoformat()}"
                ],
                "query": [
    ...
    async def _get_activity_metrics(self, session: aiohttp.ClientSession) -> dict:
        ...
        metrics = {"keystroke_rate": 0, "mouse_activity": 0, "window_switches": 0}
        ...
        try:
            ...
            query = {
                "timeperiods": [
                    f"{start_time.isoformat()}/{datetime.now().isoformat()}"
                ],
                "query": [
                    f"events = query_bucket(find_bucket('{self.window_bucket}'));",
                    "RETURN = events;",
    ...
    def _apply_privacy_filters(self, snapshot: ActivitySnapshot) -> ActivitySnapshot:
        ...
        if any(
            excluded in snapshot.active_app.lower() for excluded in self.excluded_apps
        ):
    ...
    def analyze_activity_pattern(self) -> ActivityPattern | None:
    ...
    def _calculate_pattern_indicators(
        self, activities: list[ActivitySnapshot]
    ) -> dict[str, float]:
    ...
    def _determine_pattern_type(self, indicators: dict[str, float]) -> str:
        ...
        if keystrokes > 80 and switches < 2 and consistency > 0.8:
            return "deep_focus"
        if keystrokes > 50 and switches < 4:
            return "focused_work"
        if switches > 6 and keystrokes < 40:
            return "exploring"
        if afk > 180:
            return "break_time"
        if switches > 8 and consistency < 0.3:
            return "context_switching"
        if keystrokes < 20 and switches < 2:
            return "reading"
        return "mixed_activity"

    def _calculate_pattern_confidence(
        self, indicators: dict[str, float], pattern_type: str
    ) -> float:
    ...
    def get_activity_context(self) -> dict[str, Any]:
        ...
        context = {
            "active": self.active,
            "current_pattern": (
                self.current_pattern.__dict__ if self.current_pattern else None
            ),
    ...
    def _get_recent_patterns(self) -> list[str]:
        ...
        return [p["pattern"].pattern_type for p in self.pattern_history[-5:]]
    ...
    def _assess_interruption_readiness(self) -> str:
        ...
        if pattern in ["deep_focus", "focused_work"]:
            return "not_ready"
        if pattern in ["exploring", "context_switching"]:
            return "ready"
        if pattern == "break_time":
            return "ideal"
        return "neutral"
