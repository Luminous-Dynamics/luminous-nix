...
os.environ["LUMINOUS_NIX_ENHANCED"] = "true"
...
try:
    request = Request(
        query="list generations", context={"frontend": "cli", "dry_run": True}
    )
...
if os.path.exists(cli_path):
    try:
        ...
        if (
            "Using Python backend" in result.stdout
            or "Using Python backend" in result.stderr
        ):
        ...
        if (
            "enhanced backend" in result.stdout.lower()
            or "enhanced backend" in result.stderr.lower()
        ):
