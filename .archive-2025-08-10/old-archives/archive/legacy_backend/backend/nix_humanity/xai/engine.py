...
class CausalXAIEngine:
    ...
    def explain_decision(
        ...
        try:
            ...
            logger.info(
                "Building causal model for decision: %s %s",
                decision.action,
                decision.target,
            )
    ...
    def explain_simple(self, decision: Decision, persona: str | None = None) -> str:
        """
        Get just the simple explanation for a decision.

        Convenience method for when only a one-sentence explanation is needed.
        """
        explanation = self.explain_decision(decision, persona, ExplanationLevel.SIMPLE)
        return explanation.simple

    def get_primary_reason(self, decision: Decision) -> CausalFactor | None:
        """
    ...
    def compare_alternatives(
        self, decision: Decision, alternatives: list[str]
            ...
            try:
                ...
                explanations.append(
                    AlternativeExplanation(
                        ...
                        key_factor=self._get_differentiating_factor(
                ...
                logger.warning(
                    "Failed to analyze alternative %s: %s", alt_decision.target, str(e)
                )
                explanations.append(
                    AlternativeExplanation(
                        alternative=alt_decision.target,
    ...
    def _extract_causal_path(
        self, causal_model: Any, decision: Decision
    ...
    def _adapt_explanation_for_persona(
        self, explanation: Explanation, persona: str | None
    ) -> Explanation:
        ...
        adapted = Explanation(
            ...
            detailed=(
                ...
                if config.explanation_level != ExplanationLevel.SIMPLE
                else explanation.simple
            ),
            expert=(
                explanation.expert
                if config.explanation_level == ExplanationLevel.EXPERT
                else {}
            ),
            ...
            alternatives_rejected=(
                explanation.alternatives_rejected if config.include_alternatives else []
            ),
            causal_graph=(
    ...
    def _update_average_time(self, new_time: float) -> None:
        ...
        self.metrics["average_generation_time"] = (
            old_avg * (count - 1) + new_time
        ) / count

    def _generate_fallback_explanation(
        self, decision: Decision, error: str
    ) -> Explanation:
        ...
        simple = (
            f"I decided to {decision.action} {decision.target} based on your request."
        )
        detailed = (
    ...
    def _compare_effects(
        self, chosen: Decision, alternative: Decision, alt_effects: CausalEffects
    ) -> str:
    ...
    def _get_differentiating_factor(
        self, chosen: Decision, alternative: Decision
    ) -> CausalFactor | None:
