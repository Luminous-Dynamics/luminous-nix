...
from nixos_backend import NixOSOperationHandler
...
class NaturalLanguageExecutor:
        ...
        for pattern in dangerous_patterns:
            if re.search(pattern, request.query):
                return ExecutionResponse(
                    ...
                    suggestions=[
                        "Please rephrase your request without special characters"
                    ],
                )
    ...
    async def _handle_install(
        self, intent: dict, request: ExecutionRequest
    ) -> ExecutionResponse:
    ...
    async def _handle_update(self, request: ExecutionRequest) -> ExecutionResponse:
        ...
        if request.dry_run:
            ...
            details = {
                "operations": ["nix-channel --update", "nixos-rebuild switch"],
                "dry_run": True,
            }
        else:
    ...
    async def _handle_rollback(self, request: ExecutionRequest) -> ExecutionResponse:
        ...
        if request.dry_run:
            ...
            if current_gen and len(generations) > 1:
                previous = (
                    generations[1] if generations[0].get("current") else generations[0]
                )
        ...
        return ExecutionResponse(
            ...
            details=result,
            personality_response=self._add_personality(
                result["message"], request.personality
            ),
        )
    ...
    async def _handle_search(
        ...
        message = "To search for packages, use:\n"
        message += f"`nix search nixpkgs {search_term}`\n"
        message += "Or visit https://search.nixos.org"
...
class InteractiveExecutor:
    ...
    async def run(self):
        ...
        print(
            f"Mode: {'DRY RUN' if self.dry_run else 'LIVE'} | Personality: {self.personality}"
        )
        print("=" * 50)
