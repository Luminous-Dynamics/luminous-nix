...
class PerformanceDemo:
    ...
    async def benchmark_operation(
        self, backend, operation: NixOperation, name: str
    ) -> dict[str, Any]:
    ...
    async def run_demo(self):
        ...
        for nix_op, subprocess_cmd, name in operations:
            ...
            if os.geteuid() == 0 or "dry" in name.lower():
                ...
                subprocess_result = await self.benchmark_subprocess(
                    subprocess_cmd, name
                )
            ...
            # Test native API
            native_result = await self.benchmark_operation(
                backend, nix_op, f"{name} (Native API)"
            )
...
async def main():
    ...
    if os.geteuid() != 0:
        ...
        print(
            "   For full demo, run with sudo or set LUMINOUS_NIX_ALLOW_UNPRIVILEGED=true"
        )
