...
# Import our modules
from emotion_system import EmotionNLPConnector, EmotionSystem, EmotionType
from gesture_system import (
    GestureNLPConnector,
    GestureRenderer,
    GestureSystem,
    GestureType,
...
class EnhancedAvatar:
    ...
    def draw(self, current_gesture=None):
        ...
        if (
            current_gesture
            and current_gesture.type == GestureType.POINTING
            and current_gesture.target_point
        ):
        ...
        for i in range(3):
            ...
            pygame.gfxdraw.filled_circle(
                self.screen, avatar_x, avatar_y, glow_radius, glow_color + (glow_alpha,)
            )
            pygame.gfxdraw.aacircle(
                self.screen, avatar_x, avatar_y, glow_radius, glow_color
            )

        # Draw main orb
        pygame.gfxdraw.filled_circle(
            self.screen, avatar_x, avatar_y, self.radius, self.color
        )
        pygame.gfxdraw.aacircle(
            self.screen, avatar_x, avatar_y, self.radius, self.color
        )
    ...
    def draw_ui_hints(self, current_gesture):
        ...
        if hasattr(self, "current_emotion"):
            emotion_text = font.render(
                f"Emotion: {self.current_emotion}", True, (200, 200, 200)
            )
        ...
        if current_gesture:
            gesture_text = font.render(
                f"Gesture: {current_gesture.type.value}", True, (200, 200, 200)
            )
    ...
    def draw_mock_ui_elements(self):
        ...
        for elem in elements:
            ...
            label_rect = label.get_rect(
                center=(elem["x"] + elem["w"] // 2, elem["y"] + elem["h"] // 2)
            )
...
class EmbodiedNixWithGestures:
    ...
    async def process_input(self, user_input: str) -> dict:
        ...
        emotion_result = self.emotion_connector.process_nlp_result(
            intent=intent["action"], confidence=confidence, success=None
        )
        ...
        while running:
            ...
            if self.current_gesture:
                offset = self.gesture_system.get_gesture_offset(
                    self.current_gesture, (self.avatar.x, self.avatar.y)
                )
