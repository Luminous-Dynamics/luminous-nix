...
class GestureSystem:
    ...
    def __init__(self):
        self.current_gesture: Gesture | None = None
    ...
    def find_element_by_name(self, name: str) -> ScreenElement | None:
    ...
    def find_element_at_position(self, x: int, y: int) -> ScreenElement | None:
    ...
    def get_pointing_angle(
        self, avatar_pos: tuple[int, int], target_pos: tuple[int, int]
    ) -> float:
    ...
    def get_gesture_offset(
        self, gesture: Gesture, avatar_pos: tuple[int, int]
    ) -> tuple[float, float]:
        ...
        if gesture.type == GestureType.WAVING:
            # Side to side motion
        ...
        if gesture.type == GestureType.THINKING:
...
class GestureNLPConnector:
    ...
    def process_input(self, text: str, intent: str) -> GestureType | None:
        ...
        if any(word in text_lower for word in self.gesture_triggers["pointing"]):
            ...
            if "firefox" in text_lower:
                self.gesture_system.start_gesture(
                    GestureType.POINTING, target="Firefox", duration=1.5
                )
                return GestureType.POINTING
            if "terminal" in text_lower:
                self.gesture_system.start_gesture(
                    GestureType.POINTING, target="Terminal", duration=1.5
                )
                return GestureType.POINTING
            if "settings" in text_lower:
                self.gesture_system.start_gesture(
                    GestureType.POINTING, target="Settings", duration=1.5
                )
                return GestureType.POINTING

        # Check other gesture triggers
        for gesture_name, triggers in self.gesture_triggers.items():
            if gesture_name != "pointing" and any(
                word in text_lower for word in triggers
            ):
        ...
        if intent == "install_package" and "success" in text_lower:
...
class GestureRenderer:
    ...
    def render_pointing_indicator(
        ...
        angle = math.atan2(target_pos[1] - avatar_pos[1], target_pos[0] - avatar_pos[0])
        ...
        for i in range(num_dots):
            ...
            pygame.draw.circle(
                screen, (255, 255, 100, alpha), (int(dot_x), int(dot_y)), int(radius)
            )
    ...
    def render_gesture_particles(
        self, screen, avatar_pos: tuple[int, int], gesture: Gesture
    ):
...
if __name__ == "__main__":
    ...
    gesture_system.start_gesture(GestureType.POINTING, target="Firefox", duration=2.0)
    ...
    for i in range(20):
        ...
        if gesture:
            ...
            print(
                f"Frame {i+1}: Progress={gesture.progress:.2f}, "
