...
class NixOSBackend:
    ...
    async def build_configuration(self, attr: str | None = None) -> AsyncIterator[dict]:
        """
        ...
        try:
            ...
            path = await asyncio.to_thread(
                nix.build, build_attr, BuildAttr(attr=build_attr, flake=self.flake_path)
            )
    ...
    async def rollback(self) -> OperationResult:
        ...
        try:
            if NIXOS_REBUILD_AVAILABLE:
                await asyncio.to_thread(nix.rollback, self.profile)
                return OperationResult(
                    success=True,
...
class NixOSOperationHandler:
        ...
        for key, value in entities.items():
            if isinstance(value, str) and len(value) > 500:
                return {
                    ...
                    "error": "Entity values must be under 500 characters",
                }
    ...
    async def _handle_update(self) -> dict:
        ...
        response = {"intent": "system.update", "steps": []}
