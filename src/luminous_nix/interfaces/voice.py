...
class VoiceInterface:
    ...
    def __init__(
        self,
        service: Optional[LuminousNixService] = None,
        ...
        Args:
            service: LuminousNixService instance
        ...
        if service is None:
            options = ServiceOptions(
                execute=False,  # Default to dry-run for safety
                interface="voice",
                verbose=False
            )
    ...
    async def process_voice_command(self, text: str) -> Response:
        """
        Process voice command through service layer.
                    ...
                    if self.config.wake_word:
                        if self.config.wake_word.lower() in text.lower():
                            ...
                            if command:
                                ...
                                # Speak response
                                if response.success:
                                    self.speak(response.text or "Command completed successfully")
                                else:
                                    self.speak(f"Sorry, {response.text or 'Command failed'}")
                    else:
                        ...
                        if response.success:
                            ...
                            self.speak(f"I had trouble with that: {response.text or 'Command failed'}")
