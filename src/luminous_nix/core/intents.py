...
class IntentRecognizer:
    ...
    def _load_patterns(self):
        ...
        self.install_patterns = [
            ...
            r"\b(can you|please|could you)\s+(install|add)\s+(.+?)(?:\s+package)?$",
            r"\bI\s+(need|want|would like)\s+(.+?)(?:\s+package)?$",
            r"\bI\s+want\s+to\s+install\s+(.+?)(?:\s+package)?$",
            r"\bget\s+(?!rid\s+of)(\S+)",  # "get" but not "get rid of"
        ]
        ...
        self.update_patterns = [
            ...
            r"\bupgrade\s+all\b",
            r"\b(update|upgrade)\s+(\S+)",  # Update specific package
        ]

        # Search patterns
        self.search_patterns = [
            r"\b(search|find|look for)\s+(for\s+)?(.+?)(\s+package)?$",
            r"\bis\s+there\s+(.+?)(\s+package)?$",
        ...
        self.help_patterns = [
            ...
            r"\blist\s+available\s+commands\b",  # More specific pattern for "list available commands"
            r"\bavailable\s+commands\b",
        ]

        # Remove/uninstall patterns
        self.remove_patterns = [
            r"\b(remove|uninstall|delete)\s+(.+?)(\s+from\s+my\s+system)?$",
            r"\b(can you|please|could you)\s+(remove|uninstall|delete)\s+(.+)",
            r"\bget\s+rid\s+of\s+(.+)",
            r"\bI\s+don\'?t\s+want\s+(.+)\s+anymore",
        ]
    ...
    async def recognize(self, text: str, context: dict[str, Any]) -> Intent:
        ...
        # 4. Unknown intent
        return Intent(
            type=IntentType.UNKNOWN, entities={}, confidence=0.0, raw_text=text
        )
    ...
    def _match_patterns(self, text: str) -> Intent | None:
        ...
        for pattern in self.install_patterns:
            if match := re.search(pattern, text, re.IGNORECASE):
                ...
                # Remove "the" prefix
                if package.startswith("the "):
                    package = package[4:]
                # Remove common suffixes
                for suffix in ["package", "editor", "browser", "tool", "program", "app", "application"]:
                    if package.endswith(f" {suffix}"):
                        package = package[:-len(suffix)-1]
        ...
        for pattern in self.update_patterns:
            if match := re.search(pattern, text, re.IGNORECASE):
                ...
                if len(match.groups()) >= 2 and match.group(2) not in ["system", "nixos", "my", "everything", "all"]:
        ...
        for pattern in self.search_patterns:
            if match := re.search(pattern, text, re.IGNORECASE):
                # Extract search query from the last non-empty group
                query = None
                for group in reversed(match.groups()):
                    if group and group not in ["for", "package", "packages", "programs"]:
                        query = group
                        break
                
                if not query:
                    # Fallback to raw text minus the verb
                    query = text.replace("search", "").replace("find", "").replace("look for", "").strip()
                
                return Intent(
                    type=IntentType.SEARCH_PACKAGE,
                    entities={"package": query.strip() if query else text},
                    confidence=0.8,
                    raw_text=text,
                )
        ...
        for pattern in self.remove_patterns:
            if match := re.search(pattern, text, re.IGNORECASE):
                ...
                # Clean up the package name
                package = package.lower().strip()
                # Remove common suffixes like "package", "editor", "browser", etc.
                for suffix in ["package", "editor", "browser", "tool", "program", "app", "application"]:
                    if package.endswith(f" {suffix}"):
                        package = package[:-len(suffix)-1]
