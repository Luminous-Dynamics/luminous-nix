...
try:
    # This would normally check for nixos-rebuild-ng availability
    # For now, we'll check for NixOS version 25.11+
    nixos_version = check_nixos_version()
    NATIVE_API_AVAILABLE = nixos_version and nixos_version >= (25, 11)
except Exception:
...
class NativeOperationsManager:
    ...
    def _check_compatibility(self):
        ...
        if not self.compatible:
            self.override = (
                os.environ.get("LUMINOUS_NIX_FORCE_NATIVE_API", "false").lower()
                == "true"
            )
            if not self.override:
                raise RuntimeError(
                    f"NixOS {self.nixos_version} does not support native API. "
                    "Upgrade to 25.11+ or set LUMINOUS_NIX_FORCE_NATIVE_API=true"
                )
