    ...
    def _init_advanced_api(self):
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"⚠️ Advanced nixos-rebuild API not available: {e}")
                logger.warning(
                    "📝 Requires Python 3.13+ or proper nix-shell environment"
                )
        except ImportError:
            self.advanced_available = False
...
"""
    ...
    async def _build_vm_image(self, options: dict[str, Any]) -> NativeOperationResult:
        """Build VM image"""
        from luminous_nix.core.native_operations import NativeOperationType
