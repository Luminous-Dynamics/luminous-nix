...
try:
    from nix_for_humanity.security import InputValidator, SecurityLevel, ValidationError
    ...
    if os.environ.get("NIX_HUMANITY_DEBUG", "").lower() == "true":
        print(
            "Debug: Security module not available. Running without enhanced security."
        )

# Import new v1.0 improvements
try:
    from nix_for_humanity.core.first_run_wizard import FirstRunWizard, run_if_needed
    ...
    if os.environ.get("NIX_HUMANITY_DEBUG", "").lower() == "true":
        print(
            "Debug: v1.0 improvements not available. Running without enhanced features."
        )

# Import our knowledge engines and systems
try:
    from nix_for_humanity.knowledge import (
        ModernNixOSKnowledgeEngine,
        NixOSKnowledgeEngine,
    )
except ImportError:
    try:
        from nix_for_humanity.core.knowledge import (
            NixKnowledgeBase as NixOSKnowledgeEngine,
        )

        ModernNixOSKnowledgeEngine = NixOSKnowledgeEngine  # Use same as fallback
    except ImportError:
        # Fallback to script-based knowledge engine
        try:
            spec = importlib.util.spec_from_file_location(
                "nix_knowledge_engine",
                os.path.join(script_dir, "nix-knowledge-engine.py"),
            )
            ...
            ModernNixOSKnowledgeEngine = NixOSKnowledgeEngine
        except:
            ...
            class NixOSKnowledgeEngine:
                ...
                def get_help(self, *args, **kwargs):
                    return "NixOS knowledge engine not available"
...
try:
    ...
    from nix_for_humanity.core.plugin_manager import get_plugin_manager
...
try:
    ...
    spec = importlib.util.spec_from_file_location(
        "package_cache_manager", os.path.join(script_dir, "package-cache-manager.py")
    )
...
class ProgressSpinner:
    ...
    def _spin(self):
        while self.running:
            ...
            sys.stdout.write(
                f"\r{self.spinner_chars[self.current]} {self.message}{time_info}"
            )
            sys.stdout.flush()
...
class UnifiedNixAssistant:
    def __init__(self):
        ...
        self.personality = (
            "friendly"  # Can be: minimal, friendly, encouraging, technical, symbiotic
        )
    ...
    def try_general_python_backend(self, query: str) -> bool:
        ...
        # Check feature flag
        if os.getenv("NIX_HUMANITY_PYTHON_BACKEND", "").lower() not in (
            "true",
            "1",
            "yes",
        ):
        ...
        try:
            ...
            # Add parent directory to Python path so we can import nix_for_humanity.core as backend
    ...
    def execute_with_bridge(self, intent: dict, operation: str = "command") -> tuple:
        ...
        spinner = ProgressSpinner(
            progress_info["message"], progress_info["estimated_time"]
        )
        ...
        try:
            ...
            if result.stdout:
                try:
                    ...
                    if not response.get("success", False) and response.get(
                        "suggestions"
                    ):
            ...
            return (
                False,
                "",
                "Command timed out. This might be normal for large operations.",
            )
        except Exception as e:
            if self.show_progress:
                spinner.stop()
    ...
    def execute_with_progress(self, command: str, operation: str = "command") -> tuple:
        ...
        for attempt in range(self.retry_count):
            try:
                ...
                if attempt < self.retry_count - 1:
                    spinner.message = (
                        f"{progress_info['message']} (Retry {attempt + 1})"
                    )
            except Exception as e:
                # Handle with graceful degradation if available
                if V1_IMPROVEMENTS_AVAILABLE:
                    try:
                        nix_error = degradation_handler.handle_resource_error(
                            e, operation
                        )
    ...
    def check_home_manager_installed(self) -> bool:
        ...
        try:
            result = subprocess.run(
                ["home-manager", "--version"], capture_output=True, timeout=5
            )
            return result.returncode == 0
        except Exception:
            return False

    def execute_search(self, query: str, package: str | None = None):
    ...
    def _perform_search(self, search_term: str) -> dict:
        ...
        if SECURITY_AVAILABLE:
            ...
            try:
                ...
                if validated.modifications:
                    print("Note: Search term sanitized for security.")
        ...
        try:
            ...
            if RICH_AVAILABLE and self.visual_mode:
                console.print(
                    "[yellow]Search timed out. Try a more specific search term.[/yellow]"
                )
            else:
    ...
    def validate_package_name(self, package: str) -> tuple:
        ...
        try:
            ...
            return (
                True,
                f"Could not validate package '{package}' in time, proceeding anyway",
            )
        except Exception as e:
            if spinner:
                spinner.stop()
            ...
            return (
                True,
                f"Could not validate package '{package}' ({str(e)}), proceeding anyway",
            )
    ...
    def execute_remove(self, package: str):
        ...
        result = subprocess.run(
            ["nix", "profile", "list", "--json"], capture_output=True, text=True
        )
        ...
        if not self.dry_run and not hasattr(self, "skip_confirmation"):
            if not self.confirm_action(
                "removal", f"Remove package '{package}' (index {package_index})"
            ):
                print("\n❌ Removal cancelled.")
                return
        ...
        if (
            ...
            and hasattr(self, "current_command_id")
            and self.current_command_id
        ):
    ...
    def execute_update(self, prefer_no_sudo: bool = False):
        ...
        if prefer_no_sudo or not self._is_nixos():
            ...
            if not self.dry_run and not hasattr(self, "skip_confirmation"):
                if not self.confirm_action(
                    "system update", "Update entire NixOS system (requires sudo)"
                ):
                    print("\n❌ Update cancelled.")
                    return
            ...
            channel_success, _, channel_error = self.execute_with_progress(
                "sudo nix-channel --update", "channel update"
            )
        ...
        if (
            ...
            if success:
                ...
                self.learning_system.learn_user_preference(
    ...
    def execute_install(self, package: str, method: str = "nix-profile"):
        ...
        if method == "nix-profile":
            ...
            print("1. Edit ~/.config/home-manager/home.nix")
            print(f"2. Add to home.packages: {package}")
            print("3. Run: home-manager switch")
            return

        # Execute the command - use bridge if available
        if hasattr(self, "use_bridge") and self.use_bridge:
            intent = {"action": "install_package", "package": package}
    ...
    def answer(self, query: str):
        ...
        if self.learning_enabled and hasattr(self, "learning_system"):
            ...
            if success_rate < 0.5 and success_rate > 0:  # Low success rate
                print(
                    f"⚠️  Note: This command has a {success_rate:.0%} success rate in your history"
                )
        ...
        if self.plugin_manager:
            ...
            plugin_result = self.plugin_manager.handle_intent(
                intent["action"], plugin_context
            )
            ...
            if plugin_result and plugin_result.get("success"):
                ...
                final_response = self.enhance_response(
                    final_response, query, self.personality
                )

                # Add dry-run notice if applicable
                if self.dry_run and intent["action"] in [
                    "install_package",
                    "remove_package",
                    "update_system",
                    "update_system_sudo",
...
def print_usage():
    ...
    if RICH_AVAILABLE:
        ...
        console.print(
            "  --symbiotic       🧬 Co-evolutionary mode (admits uncertainty)"
        )
        ...
        if plugin_info and plugin_info["total_plugins"] > 0:
            ...
            console.print(
                f"  🎭 Personality plugins: {plugin_info['personality_plugins']}"
            )
...
def main():
    ...
    if is_interactive and not has_piped_input and not has_cli_args:
        ...
        try:
            ...
            from nix_for_humanity.tui import run as run_tui
    ...
    if len(sys.argv) >= 2 and sys.argv[1].startswith("--"):
        ...
        if sys.argv[1] in learning_commands:
            ...
            script_path = os.path.join(
                os.path.dirname(__file__), "..", "scripts", "activate-learning.py"
            )
    ...
    # Auto-execute remove commands
        ...
        if not assistant.dry_run:
            ...
            if hasattr(assistant, "use_bridge") and assistant.use_bridge:
                ...
                success, output, error = assistant.execute_with_bridge(
                    intent_for_bridge, "remove"
                )
                if success:
