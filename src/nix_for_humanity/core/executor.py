        ...
        try:
            ...
            if NATIVE_API_AVAILABLE:
                ...
                if compat["can_override"]:
                    print(
                        "⚠️ Using Native API with version override - some features may not work"
                    )
                else:
                    print(
                        f"✅ Native Python-Nix API initialized on {compat['current_version']} - 10x performance boost!"
                    )
            else:
            ...
            if permission_result.get("requires_elevation"):
                if self.progress_callback:
                    self.progress_callback(
                        "🔐 This operation requires elevated privileges", 0.1
                    )
