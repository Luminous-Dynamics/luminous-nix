...
class IntentEngine:
    ...
    def __init__(self):
        ...
        self.patterns = {
            ...
            IntentType.CONFIG: [
                (r'^(configure|config|set\s+up)\s+(.+)$', 2),
                (r'^enable\s+(.+)$', 1),
                (r'^(help\s+me\s+)?(set\s+up|configure)\s+(.+)$', 3),
                (r'^how\s+do\s+i\s+configure\s+(.+)$', 1),
                (r'^setup\s+(.+)$', 1),
            ],
        }
    ...
    def recognize(self, text: str) -> Intent:
        ...
        if not isinstance(text, str):
            return Intent(
                ...
                target='',
                confidence=0.0,
                raw_input=str(text) if text else '',
                metadata={'original_input': str(text) if text else ''}
            )
        
        # SECURITY: Length and safety checks
        if len(text) > 1000:  # Prevent DoS attacks with very long strings
            return Intent(
                type=IntentType.UNKNOWN,
                entities={},
                target='',
                confidence=0.0,
                raw_input=text[:100] + '...',
                metadata={'original_input': text[:100] + '...'}
            )
        
        # SECURITY: Check for dangerous patterns before regex processing
        dangerous_patterns = [
        ...
        # Normalize input (after security checks)
        ...
        for intent_type, patterns in self.patterns.items():
            for pattern, target_group in patterns:
                ...
                if match:
                    ...
                    # For system updates, target should be None, not empty string
                    final_target = target
                    if intent_type == IntentType.UPDATE and target_group == 0:
                        final_target = None
    ...
    def extract_package_name(self, text: str) -> Optional[str]:
        ...
        if len(text) > 500:  # Prevent DoS with very long package names
            return None
