...
class ConsciousnessOrb(Widget):
    ...
    def on_mount(self) -> None:
        ...
        self.set_interval(1 / 60, self.update_animation)  # 60fps animation
    ...
    def animate_breathing(self) -> None:
        ...
        target_value = 0.3 if self.ai_state == AIState.IDLE else 0.8
        # FIXME: Textual animation API issue - for now just set the value directly
        self.pulse_intensity = target_value
        # Schedule the next breathing cycle
        self.set_timer(2.0 / self.breathing_rate, self.animate_breathing)
    ...
    def pulse(self, intensity: float = 1.0):
        ...
        self.animate(
            "pulse_intensity", value=intensity, duration=0.3, easing="out_cubic"
        ).on_complete = lambda: self.animate(
            "pulse_intensity", value=0.5, duration=0.5, easing="in_cubic"
        )
