...
class TestNLPPipelineSimple(unittest.TestCase):
    ...
    def test_confidence_scoring(self):
        ...
        def calculate_confidence(text, matched_intent):
            ...
            if (
                (
                    matched_intent == "install_package"
                    and "install_package" in text.lower()
                )
                or matched_intent == "update_system"
                and "update_system" in text.lower()
            ):
                confidence += 0.3
        ...
        self.assertEqual(
            calculate_confidence("install firefox", "install_package"), 1.0
        )
        self.assertEqual(calculate_confidence("add package", "install_package"), 0.7)
        self.assertEqual(
            calculate_confidence(
                "this is a very long command with many words", "install_package"
            ),
            0.5,
        )
    ...
    def test_entity_extraction_patterns(self):
        ...
        self.assertEqual(
            extract_entities("remove vim"), {"package": "vim", "action": "remove"}
        )
        self.assertEqual(
    ...
    def test_tokenization_strategies(self):
        ...
        self.assertEqual(
            tokens2, ["install_package", "firefox", "chrome", "and", "vim"]
        )
    ...
    def test_command_classification(self):
        ...
        def classify_command(text):
            ...
            if any(word in text_lower for word in ["update_system", "upgrade"]):
                return "system_maintenance"
            if any(word in text_lower for word in ["search_package", "find", "list"]):
